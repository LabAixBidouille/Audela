 ...
 *
 * $Id: canon.h,v 1.1 2006-12-03 09:56:40 michelpujol Exp $
 *
 */
...
struct _CameraPrivateLibrary
{
	...
	int capture_step;	/* To record progress in interrupt
				 * reads from capture */
	canonTransferMode transfert_mode;	/* To remember what interrupt messages
...
int canon_int_capture_without_cf (Camera *camera, unsigned char **data, int *length,
			       GPContext *context);
...
int canon_int_prepare_long_exposure (Camera *camera, int remoteState, GPContext *context);
int canon_int_get_release_parameters (Camera *camera, GPContext *context);
int canon_int_set_release_parameters (Camera *camera, GPContext *context);
/*
