...
#endif
// modif michel
    ret = usb_set_configuration(port->pl->dh,port->pl->d->config->bConfigurationValue); 
    if (ret < 0) {
      gp_port_set_error (port, "gp_port_usb_open (): Could not set configuration");
