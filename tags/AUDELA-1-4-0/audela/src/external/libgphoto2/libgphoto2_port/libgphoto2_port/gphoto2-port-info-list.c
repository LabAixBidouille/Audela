...
gp_port_message_codeset (const char *codeset) {
   // modif michel
	...
	return bindtextdomain (GETTEXT_PACKAGE, codeset);
}
...
//modif michel
#if defined(WIN32)
        if( strstr(_strlwr((char *) filename), ".dll") == NULL ) {
            continue;
        }
        ...
        lib_type = (GPPortLibraryType) gp_system_dlsym (lh, "gp_port_library_type");
        lib_list = (GPPortLibraryList) gp_system_dlsym (lh, "gp_port_library_list");
        if (!lib_type || !lib_list) {
            gp_log (GP_LOG_DEBUG, "gphoto2-port-info-list",
                _("Could not find some functions in '%s' (%s)"),
...
gp_port_info_list_load (GPPortInfoList *list)
{
	...
	gp_log (GP_LOG_DEBUG, "gphoto2-port-info-list", _("Using ltdl to load io-drivers "
		"from '%s'..."),iolibs);
#ifdef WIN32
   result = gp_port_info_list_load_dir(list,iolibs);
