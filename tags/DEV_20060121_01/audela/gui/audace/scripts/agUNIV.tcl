...
#--- Definition des couleurs 
  ...
  set color(scroll)     #BBBBBB 
   ...
   set caption(acqcolor,fonc_tolérance)	"nombre de pixels de tolérance de guidage =+/- "
...
pack .test5.frame6 \
   ...
   label .test5.frame6.lab \
      -text "$caption(acqcolor,fonc_tolérance)" -bg $color(back) -fg $color(text)
...
set e1 [expr $infos(tolérance)*$infos(focale_guide)/$infos(focale_acq)/2]
set e2 [expr $e1*2]
set e3 [expr $e1*4]
set e4 [expr $e1*5]
set u $n0
