 ...
 *
 * $Id: serial.c,v 1.1 2006-12-03 09:56:41 michelpujol Exp $
 *
 ****************************************************************************/
...
canon_serial_send_msg (Camera *camera, unsigned char mtype, unsigned char dir, va_list * ap)
{
	...
	while (1) {
		...
		if (pos + len - pkt > MAX_MSG_SIZE && camera->pl->uploading != 1) {
			GP_DEBUG ("FATAL ERROR: message too big (%i)\n", pos + len - pkt);
			return -1;
		}
		memcpy (pos, str, len);
		pos += len;
...
canon_serial_ready (Camera *camera, GPContext *context)
{
	...
	for (try = 0; try < MAX_TRIES; try++) {
		if (canon_serial_send (camera, "\x55\x55\x55\x55\x55\x55\x55\x55", 8, USLEEP1)
		    < 0) {
