...
proc ::afRho::createDialog { } {
  ...
  #--- Cree la zone a renseigner nangle
   ...
   button $This.but_aide -text "$caption(afrho,aide)" -width 7 -borderwidth 2 \
      -command { ::afRho::afficheAide }
   pack $This.but_aide -in $This.frame4 -side right -anchor w -padx 3 -pady 3 -ipady 5

   #--- Fonctionnalite non utilisable sous Linux
   if { $::tcl_platform(os) == "Linux" } {
      $This.explore configure -state disabled
      $This.entrycenterx configure -state disabled
      $This.entrycentery configure -state disabled
      $This.entryradius1 configure -state disabled
      $This.entryradius2 configure -state disabled
      $This.entryrstep configure -state disabled
      $This.entryastep configure -state disabled
      $This.entrygeodist configure -state disabled
      $This.entryheliodist configure -state disabled
      $This.entrypixelsize configure -state disabled
      $This.entryexptime configure -state disabled
      $This.entryairmass configure -state disabled
      $This.entryzeropoint configure -state disabled
      $This.entryextcoef configure -state disabled
      $This.entryfluxzero configure -state disabled
      $This.entrysolarirr configure -state disabled
      $This.entrytype configure -state disabled
      $This.entryangle1 configure -state disabled
      $This.entryangle2 configure -state disabled
      $This.entrynangle configure -state disabled
      $This.entryiterate configure -state disabled
      $This.entrythreshold configure -state disabled
      $This.entryradfit1 configure -state disabled
      $This.entryradfit2 configure -state disabled
      $This.entryverbose configure -state disabled
      $This.entrydebug configure -state disabled
      $This.but_ok configure -state disabled
      $This.but_appliquer configure -state disabled
   }
...
#
proc ::afRho::afficheAide { } {
      global help

      ::audace::showHelpItem "$help(dir,analyse)" "1105afrho.htm"
}
