...
/**
 * gp_camera_capture_without_cf:
 * @camera: a #Camera
 * @file: a #CameraFile
 * @context: a #GPContext
 *
 * Captures an image that won't be stored on the camera but returned in 
 * supplied @file. 
 *
 * Return value: a gphoto2 error code
 **/
int
gp_camera_capture_without_cf (Camera *camera, CameraFile *file, GPContext *context)
{
	...
	if (!camera->functions->capture_without_cf) {
		gp_context_error (context, _("This camera can "
			"not capture image without CF."));
		CAMERA_UNUSED (camera, context);
                return (GP_ERROR_NOT_SUPPORTED);
	}

	CHECK_RESULT_OPEN_CLOSE (camera, camera->functions->capture_without_cf (
