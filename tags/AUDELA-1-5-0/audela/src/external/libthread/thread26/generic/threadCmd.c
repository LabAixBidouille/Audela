 ...
 *
 * RCS: @(#) $Id: threadCmd.c,v 1.1 2008-11-29 14:31:47 michelpujol Exp $
 * ----------------------------------------------------------------------------
 */
...
static Tcl_ObjCmdProc ThreadCopyCommandObjCmd;
    ...
    Tcl_Interp *interp; /* The current Tcl interpreter */
{
    ...
    TCL_CMD(interp, THNS"copycommand",    ThreadCopyCommandObjCmd);
 ...
 *----------------------------------------------------------------------
 *
 * ThreadCopyCommandObjCmd --
 *
 *  This procedure is invoked to process the "thread::copycommand" Tcl
 *  command. This copy a commmand to another thread .
 *
 * Results:
 *  A standard Tcl result.
 *
...
static int
ThreadCopyCommandObjCmd(dummy, interp, objc, objv)
    ClientData  dummy;          /* Not used. */
    Tcl_Interp *interp;         /* Current interpreter. */
    int         objc;           /* Number of arguments. */
    Tcl_Obj    *CONST objv[];   /* Argument objects. */
{
    ...
    /*
     * Syntax: thread::copycommand threadId command
     */
    ...
    if (thrId == Tcl_GetCurrentThread()) {
      Tcl_SetResult(interp, "can't copy command to self", TCL_STATIC);
      return TCL_ERROR;
    }

    if ( Tcl_GetCommandInfo(interp, command, &cmdInfo) != 1) {
      Tcl_SetResult(interp,"Tcl_GetCommandInfo error",TCL_VOLATILE);
      return TCL_ERROR;
    }

   tsdPtr = ThreadExistsInner(thrId);
    if ( tsdPtr == NULL) {
      Tcl_SetResult(interp,"threadId not found",TCL_VOLATILE);
      return TCL_ERROR;
   }

   // je duplique la commande dans l'interpreteur
   Tcl_CreateCommand(tsdPtr->interp,command,(Tcl_CmdProc *)cmdInfo.proc,cmdInfo.clientData,NULL);

    return TCL_OK;

usage:
    Tcl_WrongNumArgs(interp,1,objv," threadId command ");
    return TCL_ERROR;
}

