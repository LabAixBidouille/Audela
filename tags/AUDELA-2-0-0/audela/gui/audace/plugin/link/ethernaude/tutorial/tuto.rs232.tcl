#
# Mise à jour $Id: tuto.rs232.tcl,v 1.9 2010-05-23 08:31:48 robertdelmas Exp $
#
...
if {[string compare $langage french] ==0 } {
   ...
   set texte(firstdark_2) "Câblage RS232."
...
doivent être câblées. Ces broches correspondent respectivement\
aux signaux RxD (réception), TxD (transmission) et GND (masse).\n\
\n\
Il ne faut jamais brancher un câble RS232 commercial sur\
la prise DB9 au risque de détruire l'appareil à piloter !\
Il faut fabriquer un câble qui comporte uniquement les\
fils reliés aux broches décrites dans le paragraphe précédent.\
"
   ...
   set texte(firstdark_3) "\
Please note that the DB9 socket pin configuration is not compatible\
with a standard RS232 socket.  Only pins 2, 3, and 5\
must be wired.  These pins correspond respectively to\
signals RxD (receive), TxD (transmit), and GND (ground).\n\
\n\
Never plug a commercial RS232 cable in the DB9\
socket. Doing so can destroy the device to drive!\
You must make a specific cable instead with only the wires\
linked to the pins described in the previous paragraph.\
"
...
wm protocol .second WM_DELETE_WINDOW tuto_rs232_exit
set widgetDemo 1
...
image$num(imageNo) configure -file [ file join $rep db9.gif ]
...
.second.t insert end " \n " {demospace}
.second.t insert end "$texte(next_topic) $texte(tuto_21)" {demo demo-i2c}
.second.t insert end " \n " {demospace}
.second.t insert end " \n " {demospace}
