...
/* Si G est au niveau bas, l'horloge est cr�e par le chronogramme.  */
...
void th7852_read(struct camprop *cam,unsigned short *buf)
{
   ...
   for (i=0;i<jmax;i++) {
      ...
      /* On retire les cx1 premiers pixels sans num�risation */
      for (j=0;j<cx1;j++) {
      ...
      for(j=0;j<imax;j++) {
         ...
         /* Cela d�clenche automatiquement un Reset pendant environ  */
         /* 0,1us. La dur�e optimale du niveau bas de phi L devrait  */
         /* etre de l'ordre de 0,3us. */

         /* On digitalise juste avant le niveau de reset (?)     */
         /* Cela bloque l'�chantillonneur bloqueur afin d'�viter */
         /* de transmettre le Reset vers l'ampli.                */
         libcam_out(port0,(unsigned char)16); /* 00010000 */
         th7852_attente(cam,4);
         libcam_out(port0,(unsigned char)0);  /* 00000000 */
         /* Delai d'�tablissement du palier de reference */
         th7852_attente(cam,7);
         ...
         /* Maintenant on va monter le niveau de phi L pour transf�rer */
         /* les charges du pixel vers l'�tage de sortie.               */
