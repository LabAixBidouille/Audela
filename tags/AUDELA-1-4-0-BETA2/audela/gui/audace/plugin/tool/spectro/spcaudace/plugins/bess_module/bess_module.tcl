...
# Mise a jour $Id: bess_module.tcl,v 1.3 2007-03-21 22:46:49 robertdelmas Exp $
...
#*******************************************************************************
   ...
   # Va chercher les mots-clé fits dans le header du fichier sélectionné
      ...
      if { $fich_in == ""} {
         foreach motcle { OBJNAME BSS_RA BSS_DEC OBSERVER BSS_INST BSS_SITE DATE-OBS EXPTIME BSS_VHEL BSS_NORM BSS_TELL obs1 obs2 obs3 datedeb heuredeb} {
...
# On liste les mots-clé BeSS
      foreach motcle { OBJNAME BSS_RA BSS_DEC OBSERVER BSS_INST BSS_SITE DATE-OBS EXPTIME BSS_VHEL BSS_NORM BSS_TELL } {
...
#     Message console "Nom du fichier retenu: %s\n" $nomFichier
      set ::bess::fich_in $nomFichier
...
#       Message console "fichier - %s - Ok (editeur)\n" $fich_in
      ...
      wm geometry $audace(base).saisie 580x500+120+50
...
#       WM_DELETE_WINDOW ::bess::Suppression
      ...
      #--------------------------------------------------------------------------------
      # Trame du nom de fichier
      frame $t.trame1 -borderwidth 5 -relief groove
      label $t.trame1.l1 -text $text_bess(fich_in) -font $police(gras)
      entry $t.trame1.e1 -textvariable ::bess::fich_in -font $police(normal) -relief sunken
      button $t.trame1.b1 -text $text_bess(SelecFile) -command {::bess::SelectFile} -font $police(titre)
      button $t.trame1.b2 -text $text_bess(ChargeFichier) -command {::bess::ChargeFichier $::bess::fich_in} -font $police(titre)
      ...
      label $t.trame2.titre -text $text_bess(param_generaux) -font $police(titre)
      grid $t.trame2.titre -in $t.trame2 -columnspan 3 -sticky ew

      foreach champ {OBJNAME BSS_RA BSS_DEC datedeb heuredeb EXPTIME BSS_VHEL BSS_NORM BSS_TELL} {
         label $t.trame2.l$champ -text $text_bess($champ) -font $police(gras)
         entry $t.trame2.e$champ -textvariable ::bess::parametres($champ) -font $police(normal) -relief sunken
         label $t.trame2.lb$champ -text $text_bess(u_$champ) -font $police(gras)
      ...
      button $t.trame3.b1 -text $text_bess(enregistrer) -command {::bess::EnregistreSaisie $::bess::fich_in} -font $police(titre)
      button $t.trame3.b2 -text $text_bess(editer) -command {::bess::EditeConfigs} -font $police(titre)
      button $t.trame3.b3 -text $text_bess(annuler) -command {::bess::AnnuleSaisie} -font $police(titre)
   ...
   #*************************************************************************#
   proc EditeConfigs { } {
# A faire - on doit juste appeler un éditeur de texte...
#     Message console "Ca vient, Ca vient...\n"
      global audace conf
...
#       On traite maintenant en bloc tous le smots-clé
      foreach motcle { OBJNAME BSS_RA BSS_DEC OBSERVER BSS_INST BSS_SITE DATE-OBS EXPTIME BSS_VHEL BSS_NORM } {
...
#                   Message console "A MODIFIER ! - %s - %s\n" $motcle $parametres($motcle)
   ...
   #       J'initialise les valeurs qui seront mises dans le header
         ...
         set formatmotcle(BSS_VHEL) "string"
...
#                    Message console "Ok, je sauve \n"
                     buf$audace(bufNo) save1d [file join $audace(rep_images) $fich_out]
...
#               Message console "Le nom de fichier est vide !\n"
             }
          } else {
