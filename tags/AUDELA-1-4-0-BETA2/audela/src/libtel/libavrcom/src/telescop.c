...
struct telini tel_ini[] = {
   {"AvrCom",    /* telescope name */
    "AvrCom",    /* protocol name */
...
{
   avrcom_GetCurrentFITSDate_function(tel->interp,ligne,"::audace::date_sys2ut");
   return 0;
}
...
int avrcom_delete(struct telprop *tel)
{
   char s[1024];
...
int avrcom_settsl(struct telprop *tel)
{
   int h,m,sec;
   char s[1024];
   tel->tsl=avrcom_tsl(tel,&h,&m,&sec);
   sprintf(s,"puts -nonewline %s \"#SS %02d:%02d:%02d#\"",tel->channel,h,m,sec); mytel_tcleval(tel,s);
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   return 0;
}

int avrcom_setdate(struct telprop *tel)
{
   char s[1024];
   char ss[1024];
   avrcom_GetCurrentFITSDate_function(tel->interp,s,"::audace::date_sys2ut");
...
double avrcom_tsl(struct telprop *tel,int *h, int *m,int *sec)
{
   char s[1024];
   char ss[1024];
   static double tsl;
   /* --- temps sideral local */
   avrcom_home(tel,"");
   avrcom_GetCurrentFITSDate_function(tel->interp,ss,"::audace::date_sys2ut");
...
int avrcom_home(struct telprop *tel, char *home_default)
{
   char s[1024];
...
void avrcom_GetCurrentFITSDate_function(Tcl_Interp *interp, char *s,char *function)
{
