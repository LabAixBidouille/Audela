...
typedef struct {
   int type;
	...
	int sens;
	double coef_vs; /* coefficient vitesse suivi azimuth, elevation, derotateur (ADU/ratio_sideral) */
	double coef_xs; /* coefficient pointage azimuth, elevation, derotateur (ADU/deg) */
	double jdinit;
	double coef_vsm; /* coefficient vitesse suivi MOINS azimuth, elevation, derotateur (ADU/ratio_sideral) */
	double coef_vsp; /* coefficient vitesse suivi PLUS azimuth, elevation, derotateur (ADU/ratio_sideral) */
	double coef_xsm; /* coefficient pointage MOINS azimuth, elevation, derotateur (ADU/deg) */
	double coef_xsp; /* coefficient pointage PLUS azimuth, elevation, derotateur (ADU/deg) */
	double temperature;
} axis_params;
...
struct telprop {
   ...
   char home0[50];
	...
	double app_rot;
	double app_drift_ra;
	double app_drift_dec;
	double app_drift_ha;
	double app_drift_az;
	double app_drift_elev;
	double app_drift_rot;
	double app_ra_adu; 
	double app_dec_adu;
	double app_ha_adu;
	double app_az_adu;
	double app_elev_adu;
	double app_rot_adu;
	double app_drift_ra_adu;
	double app_drift_dec_adu;
	double app_drift_ha_adu;
	double app_drift_az_adu;
	double app_drift_elev_adu;
	double app_drift_rot_adu;
...
static int Thread940_Init(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]);
static int CmdThread940_radec_init(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]);
static int CmdThread940_radec_coord(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]);
static int CmdThread940_radec_goto(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]);
static int CmdThread940_appcoord(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]);
