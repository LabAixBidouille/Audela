...
/****************************************************************************/
{
	...
	int l,raL,deL,magL,raLL,magLL;
	double rmaglim,ra,mag_red;
...
int Cmd_ydtcl_mes2mes(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[])
{
	...
	double *vals=NULL,val,*cmags=NULL;
	FILE *f_in,*f_out;
		...
		for (kcam=0;kcam<256;kcam++) {
            ...
            load libyd ; yd_updatezmg c:/d/tarot/tarot6/f/pretraite/im/rrlyr/ N111120220 8 15 ; yd_refzmgmes2ascii c:/d/tarot/tarot6/f/pretraite/im/rrlyr/ N111120220
				...
				for (kzmg=0;kzmg<n_zmg;kzmg++) {
					...
					n=kk;
					/* --- on coupe � une seule �toile de reference (avant c'etait 5) --- */
					if (n<1) {
...
/****************************************************************************/
{
	...
	float mean;
	double ra,dec,somme_delta,somme_carr_delta,jd_deb,P_delta,abs_P,sign_P,somme_poids,poids_unique,w,J,K,L;
...
fclose(ff);		
		...
		/*Le seuil de variabilit� L = coef3*(coef1*<mag>+coef2) */
		coef1=1.2;
		coef2=-14.6;
		...
		for (kcam=0;kcam<256;kcam++) {
			...
			for (kref=0;kref<n_ref;kref++) {
				...
				for (kfil=0;kfil<256;kfil++) {
					...
					nmes_filtre=0;
					/* --- boucle sur les mesures d'une �toile ---*/
