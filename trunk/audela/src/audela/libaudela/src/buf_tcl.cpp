...
int decodeFittingProfileInputs(Tcl_Interp *interp, int argc, char *argv[], int& xCenter, int& yCenter,
		int& minimumRadius, int& maximumRadius, double& saturationLimit, double& readOutNoise, char* const outputLine);
 ...
 *
 */
int decodeFittingProfileInputs(Tcl_Interp *interp, int argc, char *argv[], int& xCenter, int& yCenter, int& minimumRadius, int& maximumRadius,
		double& saturationLimit, double& readOutNoise, char* const outputLine) {

	/* Decode inputs */
	if((argc < 5) || (argc > 8)) {
		sprintf(outputLine,"Usage: %s %s x0 y0 radius_min ?-subradius_max? ?saturationLimit? ?readOutNoise?",argv[0],argv[1]);
		return 1;
	}

	if(Tcl_GetInt(interp,argv[2],&xCenter) != TCL_OK) {
		sprintf(outputLine,"xCenter = %s must be an integer",argv[2]);
		return 1;
	}

	if(Tcl_GetInt(interp,argv[3],&yCenter) != TCL_OK) {
		sprintf(outputLine,"yCenter = %s must be an integer",argv[3]);
		return 1;
	}
	...
	if(argc == 5) {
		...
		if(Tcl_GetInt(interp,argv[5],&maximumRadius) != TCL_OK) {
			sprintf(outputLine,"maximumRadius = %s must be an integer",argv[5]);
			return 1;
		}

		if(maximumRadius < 0.) {
			sprintf(outputLine,"maximumRadius = %d must be positive",maximumRadius);
			return 1;
		}

		if(maximumRadius < minimumRadius) {
			sprintf(outputLine,"maximumRadius = %d must be gretater than minimumRadius = %d",maximumRadius,minimumRadius);
			return 1;
		}
		...
		if (argc == 6) {
			...
			saturationLimit    = atof(argv[6]);
			if(saturationLimit < 0.) {
				sprintf(outputLine,"saturationLimit = %f must be positive",saturationLimit);
				return 1;
			}
			readOutNoise       = 0.;
		} else {
			...
			readOutNoise    = atof(argv[7]);
			if(readOutNoise < 0.) {
				sprintf(outputLine,"readOutNoise = %f must be positive",readOutNoise);
				return 1;
			}
		}
	}
...
int cmdPsfGaussian2D(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {
	...
	resultOfFunction = decodeFittingProfileInputs(interp, argc, argv, xCenter, yCenter, minimumRadius, maximumRadius,
			saturationLimit, readOutNoise, outputLine);
