...
CBuffer::CBuffer() : CDevice()
{
   ...
   /* Cr�e une chaine vide, pour �viter un pb potentiel sur le strlen(temporaryRawFileName) de FreeBuffer() */
...
void CBuffer::SetPixels(TColorPlane plane, int width, int height, TPixelFormat pixelFormat, TPixelCompression compression, void * pixels, long pixelSize, int reverseX, int reverseY) {
   ...
   // attention: ne pas utiliser les parametres width et height pass�s en entr�s de la fonctions
   // car leur valeur d�pend de la compression.
   if ( pix->getPixelClass() == CLASS_GRAY ) {
      int naxis = 2; 
                                ...
                                double *starFlux,char *message)
{
   ...
   try {
      ...
      *fiberY = previousFiberY;
      ...
      // Mais il semblerait qu'il y ait un probl�me d'acc�s concurrent � Libtt_main(TT_PTR_STATIMA)
      // quand le thread principal fait en meme temps une statistique
      // donc je je libere le mutex � la fin de cette procedure pour limiter la 
      // probabilt� d'acces concurrent
      pthread_mutex_unlock(&mutex);
      ...
      *background   = dbgmean;
      ...
      // ----------------------------------------------------
      // je soustrais le fond de ciel estim� si le bias n'a pas �t� d�j� soustrait
      // ----------------------------------------------------
      if ( biasBuf == NULL && biasValue == 0 ) {
         for(j=0;j<height;j++) {
            imgOffset  = imgPix  + j * width;
      ...
      *starY=0.;
      ...
      // je calcule le flux de l'etoile centr�e sur starX,starY 
      // et dans une ellipse de demi axe  a=3*FwmhX et b=3*FwhmY
      {
         ...
         double a2 = a*a;                 // demi grand axe de l'ellipse au carr� 
         double b2 = b*b;                 // demi petit axe de l'ellipse au carr�
         double x0 = *starX;              // x0 , y0 : centre de l'ellipse
         double y0 = *starY;

         // je definis les  bornes de balaye des pixels (en valeur enti�re) 
         ...
         if ( (imax - imin) <= 0 || (jmax - jmin) <= 0 ) {
            ...
            for(int j=jmin;j<=jmax;j++) {
               ...
               for(int i=imin;i<=imax;i++) {
                  // je verifie si le pixel est � l'int�rieur de l'ellipse 
      ...
      *starY += y1;
      ...
      if ( savedWidth != width || savedHeight != height ) {
         throw CError( "starStatus : la taille a chang�. (%d %d) => (%d %d) ", savedWidth, savedHeight, width, height);  
      }
      ...
      if ( integratedImage != 0) {
         ...
         if ( integratedImage == 2 ) {
            ...
            for(j=0;j<height;j++) {
               ...
               for(i=0;i<width;i++) {
                  imgPtr  = imgOffset+i;
                  sumPtr  = sumOffset+i;
                  // je recupere l'intensit� du pixel
            ...
            // j'ajoute l'image courante dans l'image int�gr�e
            for(j=0;j<height;j++) {
               imgOffset  = imgPix  + j * width;
               sumOffset  = sumPix +  j * width;
               for(i=0;i<width;i++) {
         ...
         // je traque le bug fantome
         if ( savedWidth != width || savedHeight != height ) {
            throw CError( "integrated : la taille a chang�. (%d %d) => (%d %d) ", savedWidth, savedHeight, width, height);  
         }
         ...
         *background   = dbgmean;
         ...
         if ( findFiber == 1 )  {
            ...
            if ( fiberInside==1 ) {
               ...
               // je traque le bug fantome
               if ( savedWidth != width || savedHeight != height ) {
                  throw CError( "sum : la taille a chang�. (%d %d) => (%d %d) ", savedWidth, savedHeight, width, height);  
               }
               ...
               if ( originSumCounter >= originSumMinCounter ) {
                  ...
                  */
                  ...
                  // je traque le bug fantome
                  if ( savedWidth != width || savedHeight != height ) {
                     throw CError( "findfiber : la taille a chang�. (%d %d) => (%d %d) ", savedWidth, savedHeight, width, height);  
                  }
                   ...
                   float *bgmean, float *bgsigma, float *contrast)
{
   ...
   try {
      ...
      *contrast = (float)dcontrast;
      ...
      // si la statistique porte sur l'image enti�re, je mets a jour les mots cls de l'image
      if ((x1==-1)&&(y1==-1)&&(x2==-1)&&(y2==-1)) {
         fLo = (float) dlocut;
         fHi = (float) dhicut;
         iMin = (int)dmini;
