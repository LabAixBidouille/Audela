/* 
 * 
 *	pxlibvs.h	External	05-Jul-2004 
 * 
 ...
 */ 
#define PXMEM_1BYTE	0x0001	    /* 1 byte per pixie (or pixel)	*/ 
#define PXMEM_2BYTE	0x0002	    /* 2 byte per pixie (or pixel)	*/ 
#define PXMEM_3BYTE	0x0003	    /* 3 byte per pixie (or pixel)	*/ 
#define PXMEM_4BYTE	0x0004	    /* 4 byte per pixie (or pixel)	*/ 
#define PXMEM_5BYTE	0x0005	    /* 5 byte per pixie (or pixel)	*/ 
#define PXMEM_6BYTE	0x0006	    /* 6 byte per pixie (or pixel)	*/ 
#define PXMEM_7BYTE	0x0007	    /* 7 byte per pixie (or pixel)	*/ 
#define PXMEM_8BYTE	0x0008	    /* 8 byte per pixie (or pixel)	*/ 
#define PXMEM_NBYTE	0x000F	    /* N byte per pixie (or pixel)	*/ 
...
struct	pxvidformat { 
	...
	long	pixelclkfreq[2];    /* pixel clock frequency, MHz, rational	*/ 
	long	vperiod[2];	    /* V period, seconds, rational		*/ 
	long	hperiod[2];	    /* H period, seconds, rational		*/ 
	long	framerate[2];	    /* frames per sec, rational 		*/ 
     ...
     */ 
    long    pixelwidth[2];  /* X axis width of 1 pixel. 0 if unknown	*/ 
    long    pixelheight[2]; /* Y axis height of 1 pixel. 0 if unknown	*/ 
    long    pixeldepth[2];  /* not used. should be 0			*/ 
    ...
    long    nosynctimeout;	/* no sync timeout after ... milliseconds   */ 
	...
	int	pixdopes[PXMAX_LOVC];	/* internal			    */ 
			    ...
			    /* PIXCI D/D24/D32/D2X/CL1/CL2/SI:		    */ 
...
struct	pxviddim {	    /* common control over x and y			    */ 
    ...
    int     vidsamples;     /* video pixels per line/column			    */ 
			    ...
			    /* XC D/D24/D32/A/D2X/CL1/CL2/SI allow only equality    */ 
...
struct pxvidphys { 
    ...
    uint32  startadrs [PXMAX_FIELDS][PXMAX_PIXIES]; /* start address		*/ 
    uint32  endadrs   [PXMAX_FIELDS][PXMAX_PIXIES]; /* end address, if used	*/ 
    ...
    uint32  bufadrs;	    /* start address			    */ 
    uint32  bufsize;	    /* next buffer is at bufadrs+bufsize    */ 
...
struct pxvidmem { 
    ...
    uint32  space[PXMAX_UNITS]; 
    uint32  start[PXMAX_UNITS]; 
    uint32  length[PXMAX_UNITS];    /* may be larger than memory size to indicate all	*/ 
    uint32  rsvd[PXMAX_UNITS]; 
}; 
