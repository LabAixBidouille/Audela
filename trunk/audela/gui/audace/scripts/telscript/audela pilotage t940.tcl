...
# modifs du 13 SEPT 2011 	"pierre pierre"=> remplacé tel1 execute_command_x_s 1 26 1 0 0 79 par ::etel_suivi_diurne"
#							"ajout de la variable de binning" => ajouté une pose entre les rappels lents 
#							"fichier d'autoguidage" => ajout d'un fichier contenant les paramètres d'autoguidage
...
#--- axe hauteur=1 
#--- axe champ=2 
::console::affiche_resultat "[mc_date2iso8601 now] : lecture paramètres\n"
...
# FORMULE incrémentale   INC_ABS=1313400000+671811808-az2*3648333 si ALPHA_ABS < 0 ALPHA_ABS=ALPHA_ABS+ inc_tour
# FORMULE logicielle    $inc_abs=$inc-tour+$zero-abs -$az2*$inc_deg stockés dans les variables 
...
::console::affiche_resultat "[mc_date2iso8601 now] :  paramètres lus\n"
...
proc etel_grande_boucle { } {
   ...
   ::console::affiche_resultat "[mc_date2iso8601 now] : Entree dans la grande boucle\n"
	catch {exec espeak.exe -v fr "démarre feuille congue"}
   while {1==1} {
...
#################################################################################
proc  repere_absolu { } {
#calcule l'angle horaire du méridien et prend le méridien et l'équateur comme repere de base
   global caption
   global base
   global paramaltaz
...
# ATTENTION a la précession du séro absolu dont il va falloir tenir compte
   ...
   if {$paramaltaz(park)== 1 } {
	...
	::console::affiche_resultat "j 'ai ouvert le fichier photo \n"
     ...
     set paramaltaz(ds)      		[lindex $line2 6]
      	...
      	if {$s4 == 2 }  {
      	catch {exec espeak.exe -v fr "j'enregistre et je passe à l'objet suivant."}
   	...
   	if {$s4 == 3 }  {
      	...
      	#je ferme le fichier des objets enregistrés
      	close $infos(PHOTO)
      set paramaltaz(maxdelta) [expr $paramaltaz(mindelta)-  1]
      ######fin pointage auto( 
      ::stoptour 
...
#mod
	frame $base.f.b3 -bg $paramaltaz(color,back) 
   	...
   	pack $base.f.b3 -fill none -pady 2
    #********libre
    	button $base.f.b3.but13 -text "acq vitesse corrigée" -command {pecv}
...
proc paramraq2	{ } {
   ...
   set caption(fonc_xshm)	"arrêt rotation de champ 0 rotation 1 arret."
   ...
   set caption(acqcolor,fonc_executer)	"éxécuter"
...
# 		set p [expr round($p/$cla) ]
      ...
      set cb 0
                ::etel_suivi_diurne
 ####    position initiale du dérotateur de champ
...
#après les test sur le ciel les  ::console::affiche_resultat seront à supprimer
...
#       }
   ...
   proc import_fich { } {
      ...
      global paramaltaz
      set ::infos(encours) "0" 
    #création du fichier paramètres instrument "lisa infrarouge echelle"
   	cd $audace(rep_images)
	# je nomme le fichier 
	...
	set infos(finstru) [open "$fileinstru" r]
   		...
   		# je lis le fichier des paramètres instrument 
      ...
      #fin boucle while et fermeture fichier
      	...
      	#je ferme le fichier de coordonnees brute
      	close $infos(finstru)
		.guid.but5 configure -text "enregistrer données instrument" -command {export_fich }
      	.guid.but3 configure -text "RELANCER" -command ::autoguider
   }
         #################################################################################
   ...
   proc export_fich { } {
	...
	set infos(finstru) [open "$fileinstru" w]
     		# j'enregistre le fichier des paramètres instrument 		
	puts $infos(finstru) "$infos(asc_droite) $infos(declinaison) $infos(x0) $infos(y0) $infos(e5) $infos(raz) $infos(rah) "


      	close $infos(finstru)
		.guid.but5 configure -text "lecture données instrument" -command {import_fich }
      	.guid.but3 configure -text "RELANCER" -command ::autoguider
   }
   ...
   button .guid.but4 -text "tourner le porte occulaire" -command {rot_port_occ }
      pack .guid.but4 -ipadx 5 -ipady 2
       ...
       #--- button importer données fichier
   button .guid.but5 -text "importer données fichier" -command {import_fich }
...
if {1==1} {
  ...
  if { $etelsimu(simulation)== 1 }   {
   button $posinibase.fra.but3 \
      -text "Vous êtes en mode simulation" -borderwidth 4 \
      -command { retourpark }
   pack $posinibase.fra.but3 \
      -in $posinibase.fra -side left -anchor center \
      -padx 30 -pady 3
  	catch {exec espeak.exe -v fr "Démarrer les contrôleurs"}
  	after 1000
  	catch {exec espeak.exe -v fr "Démarrer les contrôleurs"}
  	after 1000
  	catch {exec espeak.exe -v fr "Démarrer les contrôleurs"}
