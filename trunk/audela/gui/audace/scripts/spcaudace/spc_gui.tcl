...
#--- Debut modif Robert
      set filenamespc_spacial [tk_getOpenFile -title "$captionspc(open_fitfile)" -filetypes [list [list "$caption(tkutil,image_fits)" "[buf$audace(bufNo) extension] [buf$audace(bufNo) extension].gz"] ] -initialdir $audace(rep_images) ]
#--- Fin modif Robert
...
### Fin modif Robert
   ...
   ::extract_profil_zone $filenamespc_spacial
...
############################################################################
   ...
   if {[info command .spc.g] == "" } { 
      ...
      .spc.menuBar.file add command -label $captionspc(loadspcfit) -command "loadspc_fit" -underline 0 -accelerator "Ctrl-O"
      .spc.menuBar.file add command -label $captionspc(loadspctxt) -command "loadspc_txt" -underline 0 -accelerator "Ctrl-T"
      ...
      bind .spc <Control-O> loadspc_fit
      bind .spc <Control-A> loadspc_txt
