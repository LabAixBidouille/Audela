...
# Mise à jour $Id: confgene.tcl,v 1.84 2010-11-08 15:42:57 myrtillelaas Exp $
#
...
namespace eval ::confChoixOutil {
   ...
   proc appliquer { visuNo } {
      ...
      #--- Je supprime toutes les entrees du menu Pretraitement
      Menu_Delete $visuNo "$caption(audace,menu,preprocess)" entries
      #--- Rafraichissement du menu Pretraitement
      ...
      Menu_Command   $visuNo "$caption(audace,menu,analysis)" "$caption(audace,menu,subfitgauss)" \
         "subfitgauss $visuNo"
      Menu_Command   $visuNo "$caption(audace,menu,analysis)" "$caption(audace,menu,scar)" \
         "scar $visuNo"
...
namespace eval ::confTLE {
   ...
   proc run { this } {
      ...
      #--- initialisation des variables
      set confgene(name_sat) ""
      set confgene(name_satcoord) ""
      set confgene(satel_date) ""
      set confgene(coord_gps) "$conf(posobs,observateur,gps)"
      set confgene(satel_ravalue) "22h07m34s25"
      set confgene(satel_decvalue) "+60d17m33s0 "
   }
   ...
   proc createDialog { } {
      ...
      frame $This.frame4 -borderwidth 0
         button $This.but_satelname1 -text "$caption(confgene,tle_satelname)" -borderwidth 2 \
            -command { ::confTLE::satelname "" } 
         pack $This.but_satelname1 -in $This.frame4 -side left -anchor w -padx 5 -pady 5 -ipady 5
      pack $This.frame4 -in $This.frame1 -side top -fill both -expand 1
      
       #--- Cree le label pour rechercher un nom ou list de nom   
         frame $This.frame7 -borderwidth 0
         button $This.but_satelname -text "$caption(confgene,tle_satelnamesearch) " -borderwidth 2 \
            -command { ::confTLE::satelname $confgene(name_sat) } 
         ...
         entry $This.ent_satelname -width 6 -relief groove \
               -textvariable confgene(name_sat) -justify center 
      ...
      #--- Coordonnes GPS  
       frame $This.frame8 -borderwidth 0
         label $This.labgps -text "$caption(confgene,gps_lieu)"
         pack $This.labgps -in $This.frame8 -side left -anchor w -padx 5 -pady 5
         entry $This.ent_gps -width 6 -relief groove -textvariable "confgene(coord_gps)"  
      ...
      #--- Date obs 
      set confgene(tu,format,dmyhmsint) [ clock format [ clock seconds ] -format "%Y-%m-%dT%H:%M:%S" -timezone :UTC ]
      frame $This.frame9 -borderwidth 0
         label $This.labdate -text "$caption(confgene,dateob): $confgene(tu,format,dmyhmsint)"
         pack $This.labdate -in $This.frame9 -anchor w -side left -padx 5 -pady 5  
         entry $This.ladatetu -borderwidth 1 -textvariable confgene(satel_date)
      ...
      frame $This.frame5 -borderwidth 0
         button $This.but_searchcoord -text "$caption(confgene,tle_satelcoord)" -borderwidth 2 \
            -command { ::confTLE::satelcoord $confgene(name_satcoord) $confgene(satel_date) $confgene(coord_gps) }
         ...
         entry $This.ent_satelsatcoord -width 6 -relief groove \
            -textvariable confgene(name_satcoord) -justify center 
      ...
      #   button $This.but_searchcoordnearest -text "$caption(confgene,tle_satelcoordnearest)" -borderwidth 2 \
      #      -command { ::confTLE::satelnearest $confgene(satel_ravalue) $confgene(satel_decvalue) $confgene(satel_date) $confgene(coord_gps) }
      #   pack $This.but_searchcoordnearest -in $This.frame10 -side left -anchor w -padx 5 -pady 5 -ipadx 5 
      #   label $This.labra -text "$caption(confgene,tle_satelra)"
      #   pack $This.labra -in $This.frame10 -side left -anchor w -padx 5 -pady 5
      #   entry $This.ent_ra -width 6 -relief groove \
      #      -textvariable confgene(satel_ra) -justify center 
      #   pack $This.ent_ra -in $This.frame10 -side left -fill both -expand 1 -padx 5 -pady 5 -ipadx 5 
      #   label $This.labdec -text "$caption(confgene,tle_sateldec)"
      #   pack $This.labdec -in $This.frame10 -side left -anchor w -padx 5 -pady 5
      #   entry $This.ent_dec -width 6 -relief groove \
      #      -textvariable confgene(satel_dec) -justify center 
   ...
   # Mettre a jour le fichier TLE des satellites artificiels
   #
   proc satelcoord { name_satcoord satel_date satel_lieu } {
   ...
   #
   # confTLE::satelnearest
   # Mettre a jour le fichier TLE des satellites artificiels
   #
   #proc satelnearest { ra dec satel_date satel_lieu } {
