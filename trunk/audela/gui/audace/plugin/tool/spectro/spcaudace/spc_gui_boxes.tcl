...
# Mise a jour $Id: spc_gui_boxes.tcl,v 1.13 2008-03-22 19:28:55 michelpujol Exp $
...
namespace eval ::param_spc_audace_crop {
   global caption
   source [ file join [file dirname [info script]] spc_gui_boxes.cap ]
}
...
#------------------------------------------------------------
# ::param_spc_audace_crop::run
#  affiche une fenetre pour valider les limites des bords nuls à supprimer
#  sur l'image déja affiche dans la visu
#  Parametres :
#     lmin :  lambda minimal par defaut
#     lmax :  lambda miximal par defaut
#  Return :
#     retourne un liste contenant les limites mini et maxi selectionnees
#     par l'utilisateur
#------------------------------------------------------------
proc ::param_spc_audace_crop::run { lmin lmax } {
   global conf audace color caption
   global spcaudace
   variable private

   #--- je memorise les limites de depart
   set private(lmin) $lmin
   set private(lmax) $lmax

   #--- j'affiche la fenetre
   fillWindow
   #--- j'initalise le resultat
   set private(result) ""
   #--- j'attend la fermeture de la fenetre
...
#
#------------------------------------------------------------
proc ::param_spc_audace_crop::fillWindow { } {
  global conf audace color caption
   global spcaudace
   variable private

   set frm ".param_spc_audace_crop"
   set private(frm) $frm
   if { [winfo exists $frm] == 1 } {
      destroy $frm
   }

   if { ![info exists ::conf(spcaudace,crop,geometry)] } {
       set ::conf(spcaudace,crop,geometry) "400x200+10+10"
   }

   set audace(param_spc_audace,crop,color,textkey) $color(blue_pad)
   set audace(param_spc_audace,crop,color,backpad) #ECE9D8
   set audace(param_spc_audace,crop,color,backdisp) $color(white)
   set audace(param_spc_audace,crop,color,textdisp) #FF0000
   set audace(param_spc_audace,crop,font,c12b) [ list {Arial} 10 bold ]
   set audace(param_spc_audace,crop,font,c10b) [ list {Arial} 10 bold ]

   #--- Cree la toplevel
   toplevel $frm -class Toplevel -bg $audace(param_spc_audace,crop,color,backpad)
   wm geometry $frm $::conf(spcaudace,crop,geometry)
   wm resizable $frm 1 1
   wm title $frm $caption(spcaudace,boxes,crop,titre)
   wm protocol $frm WM_DELETE_WINDOW "::param_spc_audace_crop::cancel"

   #--- Cree le titre
   label $frm.title \
      -font [ list {Arial} 16 bold ] -text $caption(spcaudace,boxes,crop,titre) \
      -borderwidth 0 -relief flat -bg $audace(param_spc_audace,crop,color,backpad) \
      -fg $audace(param_spc_audace,crop,color,textkey)
   pack $frm.title -in $frm -fill x -side top -pady 15

   # --- Frame du bas
   frame $frm.buttons -borderwidth 1 -relief raised -bg $audace(param_spc_audace,crop,color,backpad)
   #-- Bouton Annuler
   button $frm.cancel_button  \
      -text "$caption(spcaudace,boxes,crop,cancel_button)" \
      -bg $audace(param_spc_audace,crop,color,backpad) \
      -fg $audace(param_spc_audace,crop,color,textkey) \
      -command {::param_spc_audace_crop::cancel}
   pack  $frm.cancel_button -in $frm.buttons -side left -fill none -padx 3 -pady 3
   #-- Bouton OK
   button $frm.ok_button  \
      -font $audace(param_spc_audace,crop,font,c12b) \
      -text "$caption(spcaudace,boxes,crop,ok_button)" \
      -bg $audace(param_spc_audace,crop,color,backpad) \
      -fg $audace(param_spc_audace,crop,color,textkey) \
      -command {::param_spc_audace_crop::ok}
   pack  $frm.ok_button -in $frm.buttons -side right -fill none -padx 3 -pady 3
   pack $frm.buttons -in $frm -fill x -pady 0 -padx 0 -anchor s -side bottom


   #--- Label + Entry pour lmin
   frame $frm.lmin -borderwidth 0 -relief flat -bg $audace(param_spc_audace,crop,color,backpad)
   label $frm.lmin.label  \
      -font $audace(param_spc_audace,crop,font,c12b) \
      -text "$caption(spcaudace,boxes,crop,config,lmin) " -bg $audace(param_spc_audace,crop,color,backpad) \
      -fg $audace(param_spc_audace,crop,color,textkey) -relief flat
   pack  $frm.lmin.label -in $frm.lmin -side left -fill none
   entry  $frm.lmin.entry  \
      -font $audace(param_spc_audace,crop,font,c12b) \
      -textvariable ::param_spc_audace_crop::private(lmin) -bg $audace(param_spc_audace,crop,color,backdisp) \
      -fg $audace(param_spc_audace,crop,color,textdisp) -relief flat -width 70
   pack  $frm.lmin.entry -in $frm.lmin -side left -fill none
   pack $frm.lmin -in $frm -fill none -pady 1 -padx 12

   #--- Label + Entry pour lmax
   frame $frm.lmax -borderwidth 0 -relief flat -bg $audace(param_spc_audace,crop,color,backpad)
   label $frm.lmax.label  \
      -font $audace(param_spc_audace,crop,font,c12b) \
      -text "$caption(spcaudace,boxes,crop,config,lmax) " -bg $audace(param_spc_audace,crop,color,backpad) \
      -fg $audace(param_spc_audace,crop,color,textkey) -relief flat
   pack  $frm.lmax.label -in $frm.lmax -side left -fill none
   entry  $frm.lmax.entry  \
      -font $audace(param_spc_audace,crop,font,c12b) \
      -textvariable ::param_spc_audace_crop::private(lmax) -bg $audace(param_spc_audace,crop,color,backdisp) \
      -fg $audace(param_spc_audace,crop,color,textdisp) -relief flat -width 70
   pack  $frm.lmax.entry -in $frm.lmax -side left -fill none
   pack $frm.lmax -in $frm -fill none -pady 1 -padx 12

   #--- j'ajoute les marqueurs representant les limites
   set ymin [.spc.g axis cget y2 -min]
   set ymax [.spc.g axis cget y2 -max]
   .spc.g marker create line -name lmin -linewidth 1 -dashes dash -coords "$private(lmin) $ymin $private(lmin) $ymax" -outline red -hide no -mapx x
   .spc.g marker create line -name lmax -linewidth 1 -dashes dash -coords "$private(lmax) $ymin $private(lmax) $ymax" -outline red -hide no -mapx x
   .spc.g marker bind all <ButtonPress-1>    { ::param_spc_audace_crop::pressMarker %W  %x %y}

   #--- bind pour appiquer les valeus saisies par l'utilisateur
   bind $frm.lmin.entry  <Return> {::param_spc_audace_crop::setMarker ".spc.g" "lmin" }
   bind $frm.lmax.entry  <Return> {::param_spc_audace_crop::setMarker ".spc.g" "lmax" }
   bind $frm.lmin.entry  <FocusOut> {::param_spc_audace_crop::setMarker ".spc.g" "lmin" }
   bind $frm.lmax.entry  <FocusOut> {::param_spc_audace_crop::setMarker ".spc.g" "lmax" }
}

#------------------------------------------------------------
# setMarker
#   deplace un marker a la position saisie dans la zone de saisie
#
#------------------------------------------------------------
proc ::param_spc_audace_crop::setMarker { graph marker } {
   variable private

   set coords [$graph marker cget $marker -coords]
   set coords "$private($marker) [lindex $coords 1] $private($marker) [lindex $coords 3]"
   $graph marker configure $marker -coords "$coords"
}

#------------------------------------------------------------
# pressMarker
#   débute le deplacement d'un marqueur avec la souris
#
#------------------------------------------------------------
proc ::param_spc_audace_crop::pressMarker { graph x y} {
   variable private
   set private(currentMarker) [$graph marker get current]

   #--- je réaffecte temporairement le bind du graphe
   set private(graphMotionBind)   [bind $graph <B1-Motion>]
   set private(graphReleaseBind)  [bind $graph <ButtonRelease-1>]
   bind $graph <B1-Motion>        { ::param_spc_audace_crop::moveMarker %W  %x %y }
   bind $graph <ButtonRelease-1>  { ::param_spc_audace_crop::releaseMarker %W  %x %y }
   #--- je change la couleur
  $graph marker configure $private(currentMarker) -outline green
}

#------------------------------------------------------------
# releaseMarker
#   termine le deplacement d'un marqueur avec la souris
#
#------------------------------------------------------------
proc ::param_spc_audace_crop::releaseMarker { graph x y} {
   variable private
   $graph marker configure $private(currentMarker) -outline red
   bind $graph <B1-Motion> $private(graphMotionBind)
   bind $graph <ButtonRelease-1> $private(graphReleaseBind)
}

#------------------------------------------------------------
# moveMarker
#   deplace un marqueur avec la souris
#
#------------------------------------------------------------
proc ::param_spc_audace_crop::moveMarker { graph x y} {
   variable private

   set x [$graph axis invtransform x $x]
   if {[string length $x]>8} { set x [string range $x 0 7] }
   if { $private(currentMarker) == "lmin" } {
      #--- je verifie que le marqueur lmin est inferieur a lmax
      set lmax [lindex [$graph marker cget "lmax" -coords] 0]
      if { [expr $lmax - $x -1 ] < 0  || $x < [$graph axis cget x2 -min] } {
         return
      } else {
         set private(lmin) $x
      }
   }
   if { $private(currentMarker) == "lmax" } {
      #--- je verifie que le marqueur lmax est superieur a lmin
      set lmin [lindex [$graph marker cget "lmin" -coords] 0]
      if { [expr $x - $lmin -1 ] < 0 || $x > [$graph axis cget x2 -max] } {
         return
      } else {
         set private(lmax) $x
      }
   }
   set coords [$graph marker cget $private(currentMarker) -coords]
   set coords "$x [lindex $coords 1] $x [lindex $coords 3]"
   #--- je deplace le marqueur
   $graph marker configure $private(currentMarker) -coords "$coords"
}

#------------------------------------------------------------
# ok
#   memorise le resultat et ferme la fenetre
#
#------------------------------------------------------------
proc ::param_spc_audace_crop::ok { } {
   variable private
   set private(result) [list $private(lmin) $private(lmax)]
   closeWindow
}

#------------------------------------------------------------
# cancel
#   abandonne et ferme la fenetre
#
#------------------------------------------------------------
proc ::param_spc_audace_crop::cancel { } {
   variable private
   set private(result) ""
   closeWindow
}
...
#   ferme la fenetre
#------------------------------------------------------------
proc ::param_spc_audace_crop::closeWindow { } {
   variable private
   #--- je supprime les marqueurs
   .spc.g marker delete "lmin" "lmax"
   #--- je memorise la position
   set ::conf(spcaudace,crop,geometry) [ wm geometry $private(frm) ]
   destroy $private(frm)
}
