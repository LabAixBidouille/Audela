...
namespace eval ::atos_flatdark_gui {
   ...
   proc ::atos_flatdark_gui::createdialog { this visuNo } {
      ...
      set ::atos_gui::frame(flatdark,base) $frm1
         ...
         if {$::atos_flatdark::typecorr == "flat"} {
            ...
            entry $frmdf.path -width 90

            frame $frmdf.butfile

               button $frmdf.butfile.but_select -text "..." -borderwidth 2 -takefocus 1 \
                  -command "::atos_flatdark_gui::select_file $visuNo"

               button $frmdf.butfile.but_load -text "Open" -borderwidth 2 -takefocus 1 \
                  -command "::atos_flatdark_gui::open_file $visuNo"

               grid $frmdf.butfile.but_select $frmdf.butfile.but_load -sticky nsw
            ...
            frame $frmdf.frm_info
               ...
               set info [frame $frmdf.frm_info.info]
                  ...
                  label $info.lab -text "Statistiques sur l'image (ADU) -> "
                  ...
                  grid $info.lab $info.min $info.max $info.moy $info.stdev -sticky nw
                  
               set param [frame $frmdf.frm_info.param]

                  label $param.lab -text "% d'images pour flat maitre : "
            ...
            set use_dark [frame $frmdf.use_dark]
                   ...
                   frame  $use_dark.but
                      ...
                      checkbutton $use_dark.but.check -text "Use Dark" \
                          -variable ::atos_flatdark_gui::usedark -state normal \
                          -command "::atos_flatdark_gui::select_dark $visuNo"

                      button $use_dark.but.but_select -text "..." -borderwidth 2 -takefocus 1 \
                         -command "::atos_flatdark_gui::select_masterdark $visuNo $this"
                      grid $use_dark.but.check $use_dark.but.but_select  -sticky nsw

                   entry $use_dark.path -width 90
                   set ::atos_gui::frame(use_dark) $use_dark.path
               
                   grid $use_dark.but  -sticky nsw
                   grid $use_dark.path -sticky nsw
               
            grid $frmdf.labfile    -sticky nw
            grid $frmdf.path       -sticky nw
            grid $frmdf.butfile    -sticky nw
            grid $frmdf.frm_scroll -sticky nw
            grid $frmdf.frm_info   -sticky nw
            grid $frmdf.use_dark   -sticky nw
         ...
         set ::atos_gui::frame(flatdark,dark) $frmdf
            ...
            frame $frmdf.frm_info
               ...
               set ::atos_gui::frame(param) $param
                  ...
                  label $param.lab -text "% d'images pour Dark Maitre : "
                  entry $param.pcimg  -textvariable ::atos_flatdark_gui::pcimg -width 4
                  
                  grid $param.lab $param.pcimg -sticky nsw
   ...
   proc ::atos_flatdark_gui::start { visuNo } {
      ...
      while {$::atos_flatdark_gui::sortie == 0} {
         ...
         set stat  [buf$bufNo stat]
         lappend ::atos_flatdark_gui::datagraph(all,x)   $::atos_tools::cur_idframe
      ...
      ::console::affiche_resultat "Verification des stat dans les images en $tt secondes\n"

      # Extraction des Images
      if {$::atos_flatdark_gui::sortie == 0} {

         set tt0 [clock clicks -milliseconds]
         set nb [expr int($::atos_flatdark_gui::pcimg / 100.0 * $::atos_tools::nb_frames)]
         ...
         for {set i $pas} {$i < [expr $::atos_tools::nb_frames-$pas] } {incr i $pas} {

            if {$::atos_flatdark_gui::sortie == 1} {break}
            update
            set ::atos_tools::cur_idframe $i
            if {$::atos_tools::cur_idframe > $::atos_tools::frame_end} {break}

            ::atos_tools::next_image $visuNo novisu

            set stat  [buf$bufNo stat]
            lappend ::atos_flatdark_gui::datagraph(use,x)   $::atos_tools::cur_idframe
            lappend ::atos_flatdark_gui::datagraph(use,moy) [format "%.0f" [lindex $stat 4]]

            buf$bufNo save atos_build_df${cpt}

            incr cpt
         }
         set nbimg $cpt
         set tt [format "%.3f" [expr ([clock clicks -milliseconds] - $tt0)/1000.]]
         ::console::affiche_resultat "Extraction des $nbimg images de reference en $tt secondes\n"
      }
    ...
    # On fait des Flats : 
         ...
         set tt [format "%.3f" [expr ([clock clicks -milliseconds] - $tt0)/1000.]]
         ::console::affiche_resultat "Construction du Flat Maitre en $tt secondes\n"
   ...
   proc ::atos_flatdark_gui::graph { visuNo } {
      ...
      ::plotxy::title "Courbe de lumiere des images" 
   ...
   proc ::atos_flatdark_gui::select_masterdark { visuNo frm } {
      ...
      set masterdark [::tkutil::box_load $frm [pwd] $bufNo "1"]
