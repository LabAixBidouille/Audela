...
# Auteur : Alain KLOTZ
# Mise a jour $Id: sextractor.tcl,v 1.1 2006-11-05 12:31:27 alainklotz Exp $
#
...
proc sextractor { args } {
	global audace
	set pathbin .
	catch {set pathbin "$audace(rep_gui)/../bin"}
	set exefile "${pathbin}/sextractor.exe"
	set k [file exists "$exefile"]
	if {$k==0} {
		set exefile "${pathbin}/sex.exe"
		set k [file exists "$exefile"]
	}
	if {$k==0} {
