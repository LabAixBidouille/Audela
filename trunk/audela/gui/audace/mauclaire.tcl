...
# Mise a jour $Id: mauclaire.tcl,v 1.6 2006-10-01 22:25:35 robertdelmas Exp $
#
#-----------------------------------------------------------------------------#
...
# bm_renumfile       : renomme les fichiers de numerotation collee au nom
# bm_renameext       : renomme l'extension de fichiers en extension par defaut d'Audela
# bm_pregister_lin   : regsitration planetaire sur un point initial et final : translation lineaire
# bm_sflat           : cree un flat synthetique (image d'intensite uniforme) de nxp pixels
# bm_pretrait        : effectue le pretraitement d'une serie d'images a l'aide du dark, plu, dark de plu
# bm _sadd           : effectue la somme d'une serie d'images
# bm_somes           : effectue la somme moyenne, mediane et ssk d'une serie d'images appariees
# bm_fwhm            : calcul la largeur equivalente d'une etoile en secondes d'arc
#
#-----------------------------------------------------------------------------#
...
###############################################################################
#
# Description : se met dans le repertoire de travail d'Audace pour eviter de
...
proc bm_extract_radec { } {
   global audace
   global conf
# Par defaut, travaille dans le rep images configure dans Audela
# Ne demande aucun argument
...
###############################################################################
#
# Description : renomme les fichier de numerotation collee au nom
# Auteur : Benjamin MAUCLAIRE
...
proc bm_renumfile { args } {
   ...
   if { [llength $args] == 1 } {
      ...
      ::console::affiche_resultat "Préfixe : $pref_nom_generique\n"
      file mkdir sortie
      ...
      ::console::affiche_resultat "Fichiers renommés dans le répertoire sortie.\n"
   } else {
      ::console::affiche_erreur "Usage: bm_renumfile nom_generique de fichier a la numerotation collee.\n"
   }
}
#****************************************************************************#
...
###############################################################################
#
# Description : renomme l'extension de fichiers en extension par defaut d'Audela
# Auteur : Benjamin MAUCLAIRE
# Date creation : 17-12-2005
# Date de mise a jour : 17-12-2005
...
proc bm_renameext { args } {
   ...
   if { [llength $args] <= 2 } {
      ...
      ::console::affiche_resultat "$nbimg fichiers à renommer.\n"
      ...
      foreach fichier $liste_fichiers {
         ...
         if { [file exists ${prefixe_nom}$conf(extension,defaut)] == 0 } {
            ::console::affiche_resultat "${fichier} renomme en ${prefixe_nom}$conf(extension,defaut)\n"
...
proc bm_pregister_lin { args } {
   ...
   if {[llength $args] == 1} {
      ...
      label .benji.lab -text "Selectionnez l'objet a suivre (boîte petite)"
...
###############################################################################
#
# Description : creee un flat synthetique (image d'intensite uniforme) de nxp pixels
# Auteur : Benjamin MAUCLAIRE
...
###############################################################################
#
# Description : calcul la largeur equivalente d'une etoile en secondes d'arc
# Auteur : Benjamin MAUCLAIRE
