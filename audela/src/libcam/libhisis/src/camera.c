...
#define LOG_INFO    3
#define LOG_DEBUG   4
int hisis_log_level = LOG_NONE;

void hisis_log(int level, const char *fmt, ...);
...
/* --------------------------------------------------------- */
{
...
#endif
   hisis_log_level = LOG_NONE;

   // je recupere les parametres optionnels
   if (argc >= 5) {
      for ( kk = 3; kk < argc - 1; kk++) {
         if (strcmp(argv[kk], "-loglevel") == 0) {            
            hisis_log_level = atoi(argv[kk + 1]);
         }
      }
   }

   //libcam_strlwr(CAM_INI[cam->index_cam].name, ligne);
   strcpy(ligne,CAM_INI[cam->index_cam].name);
   hisis_log(LOG_DEBUG, "===================================="); 
   hisis_log(LOG_DEBUG, "cam_init avant connexion %s", ligne); 
   ...
   if (strcmp(ligne, "Hi-SIS22-12") == 0) {
   ...
   } else if ((strcmp(ligne, "Hi-SIS22-14") == 0)
   ...
   } else if ((strcmp(ligne, "Hi-SIS24") == 0) || (strcmp(ligne, "Hi-SIS33") == 0)
      ...
      if (argc >= 2) {
         ...
         if (strcmp(ligne2, "lpt1") == 0) {
            cam->typeport = LPT;
            cam->portnum = PORT_1;
         } else if (strcmp(ligne2, "lpt2") == 0) {
            cam->typeport = LPT;
            cam->portnum = PORT_2;
         } else if (strcmp(ligne2, "lpt3") == 0) {
            cam->typeport = LPT;
            cam->portnum = PORT_3;
         } else if (strcmp(ligne2, "isa") == 0) {
            cam->typeport = ISA;
            cam->portnum = PORT_0;
         }
      } else if (cam->typeport == LPT) {
         if (cam->interrupt == 1) {
            libcam_bloquer();
         }
         if (cam->authorized == 1) {
         ...
         sprintf(cam->msg, "Nom de camera inconnu: %s",ligne);
         result = 1;
      }
   }

   cam->hisis24_shutter.shuttermode.delay = 6;
hisis_log(LOG_DEBUG, "cam_init fin connexion %s", ligne); 
...
void cam_start_exp(struct camprop *cam, char *amplionoff)
{
   ...
   if (cam->authorized == 1) {
      ...
      if (strcmp(ligne, "Hi-SIS11") == 0) {
         cam->nb_vidages = 4;
         hisis_log(LOG_DEBUG, "cam_start_exp RAZ %s ", ligne); 
      ...
      if (strcmp(ligne, "Hi-SIS22-12") == 0) {
         ...
         if (result == 1 ) {
            hisis_log(LOG_DEBUG, "cam_start_exp timeout reset error !!!!");
         }
      }
...
void cam_stop_exp(struct camprop *cam)
{
   ...
   if (cam->authorized == 1) {
      ...
      if ((strcmp(ligne, "Hi-SIS24") == 0)
         ...
         || (strcmp(ligne, "Hi-SIS39") == 0)
         || (strcmp(ligne, "Hi-SIS48") == 0)) {
         hisis_log(LOG_DEBUG, "cam_stop_exp %s ", ligne); 
...
void cam_read_ccd(struct camprop *cam, unsigned short *p)
{
   ...
   if (cam->authorized == 1) {
      ...
      if (strcmp(ligne, "Hi-SIS11") == 0) {
         hisis_log(LOG_DEBUG, "cam_read_ccd %s ", ligne); 
...
void cam_shutter_on(struct camprop *cam)
{
   ...
   if (cam->authorized == 1) {
      ...
      if ((strcmp(ligne, "Hi-SIS24") == 0)
         ...
         hisis_log(LOG_DEBUG, "cam_shutter_on %s ", ligne);
      }
   }
}
...
void cam_shutter_off(struct camprop *cam)
{
   ...
   if (cam->authorized == 1) {
      ...
      if ((strcmp(ligne, "Hi-SIS24") == 0)
         ...
         hisis_log(LOG_DEBUG, "cam_shutter_off %s ", ligne);
      }
   }
}
...
void cam_set_binning(int binx, int biny, struct camprop *cam)
{
   ...
   cam->binx = binx;
   cam->biny = biny;
   hisis_log(LOG_DEBUG, "cam_set_binning binning x=%d binning y=%d", cam->binx, cam->biny);
}
...
void cam_update_window(struct camprop *cam)
{
   ...
   */
   ...
   cam->h = (cam->y2 - cam->y1 ) / cam->biny + 1;
   cam->y2 = cam->y1 + cam->h * cam->biny - 1;
   hisis_log(LOG_DEBUG, "cam_update_window x1=%d y1=%d x2=%d y2=%d", cam->x1, cam->y1, cam->x2, cam->y2);
}
...
int hisis11_read_image(struct camprop *cam, unsigned short *pix)
{
   ...
   for (i = 0; i < jmax; i++) {
      ...
      for (j = x; j <(cam->nb_photox + cam->nb_deadbeginphotox +
...
/* --- a verifier ---*/
{
   ...
   hisis_log(LOG_DEBUG, "hisis22_read_image12 readloops=%d ", hisis22_12_readloops); 
   
   /*  Demande de poids fort */
   libcam_out(sortie, 0x0C);
   
   ktimeout = 5000000;
   for (j = 0; j < h && result == 0 ; j++) {
      ...
      for (i = 0; i < w; i++) {
         ...
         hisis_log(LOG_DEBUG, "hisis22_read_image12 lecture ligne %d colonne %d etape 1 timeout error !!!!", j, i); 
         synchro = (res & 0x40);
         ...
         hisis_log(LOG_DEBUG, "hisis22_read_image12 lecture ligne %d colonne %d etape 2 timeout error !!!!", j, i); 
...
int hisis22_read_image14(struct camprop *cam, unsigned short *pix)
{
   ...
   for (j = 0; j < h; j++) {
      ...
      for (i = 0; i < w; i++) {
         ...
         hisis_log(LOG_DEBUG, "hisis22_read_image12 timeout fin lecture=%d ", result);
         a = (res & 0xF0) >> 4;
...
char *hisis_getlogdate(char *buf, size_t size)
{
#if defined(OS_WIN)
#ifdef _MSC_VER
...
void hisis_log(int level, const char *fmt, ...)
{
   ...
   if (level <= hisis_log_level) {
      hisis_getlogdate(buf,100);
      f = fopen("hisis.log","at+");
      switch (level) {
      case LOG_ERROR:
