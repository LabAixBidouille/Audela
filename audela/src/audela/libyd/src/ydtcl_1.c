...
/****************************************************************************/
{
   ...
   int kmes,kmes_deb,kmes_end,n_mes,kcam,kkcam,kfil,n_ref,kref,n_ref0,kzmg,n_zmg;
   int k,kk,k1,k2,n,n_val,n_groupe,kgroupe_deb,kgroupe_end,nmes_filtre,kfaux;
   unsigned char ukcam,ukfil;
   FILE *f_in,*f_out;
/*FILE *f;*/
   int codes[256];
   ...
   int minobs,temoin,rien;
   double delta_temps; /*cest le seuil de variabilite*/
/*int kvar;
double Lvar=0.;
Tcl_DString dsptr;*/
...
fclose(f);
      ...
      while (feof(f_in)==0) {
         if (fread(&htmmes,1,sizeof(struct_htmmes),f_in)>1) {
		    /*un bug du mauvais tris des htmmess
			if(htmmes.indexref-kref<0) {continue;}*/
            htmmess[k]=htmmes;
...
fclose(f);
      ...
      for (kcam=0;kcam<256;kcam++) {
		 ...
		 kkcam=codes[kcam];
		 if (kkcam<0) {
			/* --- Pas assez de mesures faites avec cette congiguration camera-filtre ---*/
			 continue;
		 }
	...
	kref=kvar;}*/
			 ...
			 temoin=1; /*montre que l'etoile possede au moins une mesure valable*/
             for (kfil=0;kfil<256;kfil++) {
				 ukfil=(unsigned char)kfil;
				 /*if (kfil==67) {
					 kfil=67;
			 ...
			 /*verifie si l'etoile possede au moins une mesure valable*/
			 if (temoin) {continue;}
			 /* --- boucle sur les mesures d'une étoile ---*/
/*if (kref==kvar) {
	kref=kvar;}*/
             /*Je trouve kgroupe_deb : il se peut que la 1ere mesure soit a -99.9*/
			 for (kmes=kmes_deb;kmes<=kmes_end;kmes++) {
                 if (htmmess[kmes].codecam!=ukcam) {
                     continue;
                 }
			 ...
			 kgroupe_deb=kmes;
			 /*il y a surement au moins une bonne mesure : pas de probleme d'ecrire kgroupe_deb=kmes*/
			 jd_deb=htmmess[kgroupe_deb].jd;
			 J=0.;
			 somme_poids=0.;
			 kfaux=0;
             for (kmes=kgroupe_deb+1;kmes<=kmes_end;kmes++) {
                 if (htmmess[kmes].codecam!=ukcam) {
					 kfaux++;
                     continue;
                 }
				 ...
				 if((htmmess[kmes].jd-jd_deb>delta_temps)||(kmes==kmes_end)) {
					 kgroupe_end=kmes-1;
			 ...
			 J=J/(somme_poids+0.0000001); /*le petit plus c'est quand il y a 1 mesure*/
