...
const struct canon_usb_cmdstruct canon_usb_cmd[] = {
   //  	description, char cmd1, cmd2, int cmd3  return_length;
 ...
 */
int 
canon_usb_clear_pipe (Camera *camera, GPContext *context ) {
   unsigned char buf2[0x40]; /* for reading from interrupt endpoint */
         
   while ( canon_usb_poll_interrupt_pipe ( camera, buf2, 2 ) > 0 );
   return GP_OK;
}
 ...
 */
int
canon_usb_capture_dialogue (Camera *camera, GPContext *context )
{
   int status;
   ...
   //modif michel
   int wait_capture_end, wait_thumb_to_pc, wait_full_to_pc, wait_thumb_to_drive, wait_full_to_drive;
   int return_length;
   int initial_long_exposure;

        /* clear this to indicate that no data is there if we abort */
   //if (return_length)
   //   *return_length = 0;
   
   GP_DEBUG ("canon_usb_capture_dialogue() transfert_mode=%d long_exposure=%d",
      camera->pl->transfert_mode,camera->pl->long_exposure);
   ...
   //modif michel
   // REMOTE_CAPTURE_THUMB_TO_PC      0x0a-0x1c, 0x08     , 0x0a-0x1d
   // REMOTE_CAPTURE_FULL_TO_PC       0x0a-0x1c, 0x0c     , 0x0a-0x1d
   // REMOTE_CAPTURE_THUMB_TO_DRIVE   0x0a-0x1c, 0x0a-0x1d, 0x0e or 0x0f
   // REMOTE_CAPTURE_FULL_TO_DRIVE    0x0a-0x1c, 0x0a-0x1d, 0x0e or 0x0f 
   
   // so capture is finished  when : 
   // message 0x0a-0x1c start of capture is received 
   // and 0x08 is received  if REMOTE_CAPTURE_THUMB_TO_PC is required 
   // and 0x0c is received  if REMOTE_CAPTURE_FULL_TO_PC is required
   // and 0x0e is received  if REMOTE_CAPTURE_THUMB_TO_DRIVE is required
   // and 0x0e is received  if REMOTE_CAPTURE_FULL_TO_DRIVE is required
   // and message 0x0a-0x1d end of capture is received 
   
   
   wait_thumb_to_pc    = (camera->pl->transfert_mode & REMOTE_CAPTURE_THUMB_TO_PC);
   wait_full_to_pc     = (camera->pl->transfert_mode & REMOTE_CAPTURE_FULL_TO_PC);
   wait_thumb_to_drive = (camera->pl->transfert_mode & REMOTE_CAPTURE_THUMB_TO_DRIVE);
   wait_full_to_drive = (camera->pl->transfert_mode & REMOTE_CAPTURE_FULL_TO_DRIVE);
   initial_long_exposure = camera->pl->long_exposure;
   if (camera->pl->md->model == CANON_CLASS_4 && wait_full_to_pc  != 0) {
      wait_capture_end   = 1;
   } else {
      wait_capture_end   = 0;       
   }
   
   while (  wait_capture_end 
      || wait_thumb_to_pc 
      || wait_full_to_pc 
      || wait_thumb_to_drive 
      || wait_full_to_drive ) {
      
      if( initial_long_exposure == 1 && camera->pl->long_exposure == 0 ) {
         GP_DEBUG ( "canon_usb_capture_dialogue:"
            " user stops external remote capture before its end");
         goto FAIL;
      ...
      case 0x08:
         GP_DEBUG ( "canon_usb_capture_dialogue:"
            " buf[4]=0x%02x buf[12]=0x%02x thumbnail captured at step %d", buf2[4], buf2[12], camera->pl->capture_step++ );
         /* Thumbnail size */
         if ( status != 0x17 )
         ...
         wait_thumb_to_pc = 0;
         GP_DEBUG ( "canon_usb_capture_dialogue:"
            "capture_end=%d thumb_to_pc=%d full_to_pc=%d thumb_to_drive=%d full_to_drive=%d", 
            wait_capture_end, wait_thumb_to_pc, wait_full_to_pc, wait_thumb_to_drive,wait_full_to_drive);
      
         //camera->pl->transfert_mode &= ~REMOTE_CAPTURE_THUMB_TO_PC;
         /* Special case for class 6 (newer protocol) and the EOS 300D */
         if ( camera->pl->transfert_mode == 0
            && ( camera->pl->md->model == CANON_CLASS_6 || camera->pl->md->usb_product == 0x3084 ) )
            goto EXIT;
         break;
      case 0x0c:
         GP_DEBUG ( "canon_usb_capture_dialogue:"
            " buf[4]=0x%02x buf[12]=0x%02x Full image captured at step %d", buf2[4], buf2[12], camera->pl->capture_step++ );
         /* Full image size */
         if ( status != 0x17 )
            GP_LOG ( GP_LOG_ERROR, _("canon_usb_capture_dialogue:"
            " bogus length 0x%04x"
         ...
         GP_DEBUG ( "canon_usb_capture_dialogue:"
            "capture_end=%d thumb_to_pc=%d full_to_pc=%d thumb_to_drive=%d full_to_drive=%d", 
            wait_capture_end, wait_thumb_to_pc, wait_full_to_pc, wait_thumb_to_drive,wait_full_to_drive);
         //camera->pl->transfert_mode &= ~REMOTE_CAPTURE_FULL_TO_PC;
         /* Special case for class 6 (newer protocol) and the EOS 300D */
         if ( camera->pl->transfert_mode == 0
            && ( camera->pl->md->model == CANON_CLASS_6 || camera->pl->md->usb_product == 0x3084 ) ) 
         {
         ...
         // modif michel pour 5D
         //if ( camera->pl->transfert_mode == 2
         //  && ( camera->pl->md->model == CANON_CLASS_6 || camera->pl->md->usb_product == 0x3084 ) )
      ...
      case 0x0a:
         if ( buf2[12] == 0x1b ) {
            ...
            wait_capture_end = 0;
            GP_DEBUG ( "canon_usb_capture_dialogue:"
               " buf[4]=0x%02x buf[12]=0x%02x end of capture step=%d", buf2[4], buf2[12], camera->pl->capture_step++ );
         }
         if ( buf2[12] == 0x1c ) {
            // modif michel
            GP_DEBUG ( "canon_usb_capture_dialogue:"
               " buf[4]=0x%02x buf[12]=0x%02x start of capture step=%d", buf2[4], buf2[12], camera->pl->capture_step++ );
            ...
            GP_DEBUG ( "canon_usb_capture_dialogue:"
               " buf[4]=0x%02x buf[12]=0x%02x unknown step=%d", buf2[4], buf2[12], camera->pl->capture_step++ );
         }
         break;
      case 0x0e:
          ...
          * storage. */
         GP_DEBUG ( "canon_usb_capture_dialogue:"
               " buf[4]=0x%02x buf[12]=0x%02x fullimage copied to CF step=%d", buf2[4], buf2[12], camera->pl->capture_step++ );
         camera->pl->capture_step++;
         ...
         //    goto FAIL;
         //}
         wait_full_to_drive = 0;
               GP_DEBUG ( "canon_usb_capture_dialogue:"
            "capture_end=%d thumb_to_pc=%d full_to_pc=%d thumb_to_drive=%d full_to_drive=%d", 
            wait_capture_end, wait_thumb_to_pc, wait_full_to_pc, wait_thumb_to_drive,wait_full_to_drive);
   

         //camera->pl->transfert_mode &= ~(REMOTE_CAPTURE_THUMB_TO_DRIVE|REMOTE_CAPTURE_FULL_TO_DRIVE);
         /* Special case for class 6 (newer protocol) and the EOS 300D */         
//			if ( camera->pl->md->model == CANON_CLASS_6 || camera->pl->md->usb_product == 0x3084 ) {
//            goto EXIT;
//         }
...
//         }         
      ...
      * fourth, since we may or may not be
      ...
      case 0x81:
         GP_DEBUG ( "canon_usb_capture_dialogue:"
               " buf[4]=0x%02x buf[12]=0x%02x turn off/on screen camera step %i", buf2[4], buf2[12], camera->pl->capture_step );
         break;
      default:
         /* Unknown */
         GP_DEBUG ( "canon_usb_capture_dialogue:"
        ...
        if ( ( camera->pl->md->model != CANON_CLASS_6
		  ...
		  && canon_usb_cmd[i].num == CANON_USB_FUNCTION_CONTROL_CAMERA_2 ) ) {
                 ...
                 * newer cameras it changes capture
                   * completion. */
                   //camera->pl->transfert_mode = payload[8];
                         ...
                         int payload_length, int display_status, GPContext *context)
{
        ...
        *data_length = 0;
        ...
        if (display_status)
                id = gp_context_progress_start (context, (float) total_data_size,
                                                _("Receiving data..."));

        if (max_data_size && (total_data_size > (unsigned int) max_data_size)) {
        ...
        *data = malloc (total_data_size);
        ...
        while (bytes_received < total_data_size) {
                ...
                if ((unsigned int) bytes_read < read_bytes)
                ...
                if (display_status)
                        gp_context_progress_update (context, (unsigned int) id, (float) bytes_received);
        }
        if (display_status)
                              ...
                              GPContext *context)
{
   ...
   */
   ...
   // modif michel   canon_usb_long_dialogue plante quand display_status=1
   /* the 1 is to show status */
   if ( camera->pl->md->model == CANON_CLASS_6 )
      result = canon_usb_long_dialogue (camera, CANON_USB_FUNCTION_RETRIEVE_CAPTURE_2, data, length,
      0, payload, payload_length, 0, context);
   else
...
// modif michel
        disk_name = canon_int_get_disk_name (camera, context);
        if( disk_name == NULL ) {
           return GP_ERROR_IO;
        }
