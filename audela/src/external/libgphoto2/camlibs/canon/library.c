...
/**
 * camera_capture_without_cf
 * @camera: the camera to affect
 * @context: context for error reporting
 *
 * Triggers the camera to capture a new image. 
 * downloads image to the host computer without storing it on
 * the camera.
 *
 * Returns: GP_OK
 *
 */
EXPORT_STATIC int
camera_capture_without_cf (Camera *camera, CameraFile *file, GPContext *context)
{
	int size, code;
	unsigned char *data;

   char RawHeader[]  = { (char) 0x49, (char) 0x49 };
   char JpgHeader[]  = { (char) 0xFF, (char) 0xD8 };
   

	GP_DEBUG ("canon_capture_without_cf() called");

	code = canon_int_capture_without_cf (camera, &data, &size, context);
	if ( code != GP_OK) {
      // context error was set by called function
   ...
   if( data != NULL ) {
      ...
      if ( strncmp(data, RawHeader , 2 ) == 0 ) {
         gp_file_set_mime_type (file, GP_MIME_CRW);
         gp_file_set_name (file, "canon_image.crw");
      } else if ( strncmp(data, JpgHeader , 2 ) == 0 ) {
         gp_file_set_mime_type (file, GP_MIME_JPEG);
         gp_file_set_name (file, "canon_image.jpg");
      } else {
 ...
 * @mode:    see canonTransferMode
 * @context: context for error reporting
 *
 * Set camera's transfert mode.  If new mode use DRIVE , verify if a card is present
 *
 * Returns: gphoto2 error code
 *
 */
...
int
set_transfert_mode (Camera *camera, canonTransferMode mode, GPContext *context)
{
   int status;
   
   GP_DEBUG ("set_transfert_mode: %d", (unsigned int) mode);   
   ...
   case GP_PORT_USB:
      ...
      break;
   case GP_PORT_SERIAL:
      GP_DEBUG ("set_transfert_mode not implemented for serial port");                   
      return GP_ERROR_NOT_SUPPORTED;
      break;
      GP_PORT_DEFAULT
   }
...
camera_get_config (Camera *camera, CameraWidget **window, GPContext *context)
{
	...
	GP_DEBUG ("camera_get_config() widget=%ld",window);

   if( *window !=  NULL ) {
      char *label;
      gp_widget_get_label(*window, &label);
      if( strcmp( label, "Time value") == 0 ) {
         res = canon_int_get_release_parameters(camera, context);
         if( res == GP_OK) {
            gp_widget_set_value(*window, &camera->pl->timeValue);
         }
       	return res;
      }
      if( strcmp( label, "Drive mode") == 0 ) {
         res = canon_int_get_release_parameters(camera, context);
         if( res == GP_OK) {
            gp_widget_set_value(*window, &camera->pl->driveMode);
         }
       	return res;
      }

      if( strcmp( label, "Quality") == 0 ) {
         res = canon_int_get_release_parameters(camera, context);
         if( res == GP_OK) {
            gp_widget_set_value(*window, camera->pl->quality);
         }
       	return res;
      }
...
// modif michel
	gp_widget_new (GP_WIDGET_TOGGLE, _("Long exposure"), &t);
	gp_widget_set_value (t,&camera->pl->long_exposure);
	gp_widget_append (section, t);

	gp_widget_new (GP_WIDGET_TOGGLE, _("Transfert Mode"), &t);
	gp_widget_set_value (t,&camera->pl->transfert_mode);
	gp_widget_append (section, t);
...
// modif michel   
	gp_widget_new (GP_WIDGET_SECTION, _("Release"), &section);
	gp_widget_append (*window, section);

	gp_widget_new (GP_WIDGET_RANGE, _("Time value"), &t);
	gp_widget_set_value (t, &camera->pl->timeValue);
	gp_widget_append (section, t);

	gp_widget_new (GP_WIDGET_TOGGLE, _("Drive mode"), &t);
	gp_widget_set_value (t, &camera->pl->driveMode);
	gp_widget_append (section, t);

	gp_widget_new (GP_WIDGET_TEXT, _("Quality"), &t);
	gp_widget_set_value (t, camera->pl->quality);
	gp_widget_append (section, t);
...
camera_set_config (Camera *camera, CameraWidget *window, GPContext *context)
{
   ...
   if( window !=  NULL ) {
      ...
      gp_widget_get_label(window, &label);
      if( strcmp( label, "Time value") == 0 ) {
         gp_widget_get_value(window, &fvalue);
         camera->pl->timeValue = fvalue;
         res = canon_int_set_release_parameters(camera, context);
         if (res != GP_OK) {
             gp_context_error (context,
				_("Dialog error while setting camera esposure parameter"));
         }
         return res;
      }
      if( strcmp( label, "Drive mode") == 0 ) {
         gp_widget_get_value(window, &ivalue);
         camera->pl->driveMode = ivalue;
         res = canon_int_set_release_parameters(camera, context);
         if (res != GP_OK) {
             gp_context_error (context,
				_("Dialog error while setting camera drive mode"));
         }
         return res;
      }
      if( strcmp( label, "Quality") == 0 ) {
         gp_widget_get_value(window, &wvalue);
         strcpy(camera->pl->quality, wvalue);
         res = canon_int_set_release_parameters(camera, context);
         if (res != GP_OK) {
             gp_context_error (context,
				_("Dialog error while setting camera quality"));
         }
         return res;
      }
   }
   ...
   if (gp_widget_changed (w)) {
	...
	gp_widget_get_child_by_label (window, _("Long exposure"), &w);
	if (gp_widget_changed (w)) {
		gp_widget_get_value (w, &ivalue);
		if (!check_readiness (camera, context)) {
			gp_context_status (context, _("Camera unavailable"));
		} else {
			if (canon_int_prepare_long_exposure (camera, ivalue, context) == GP_OK) {
				gp_context_status (context, _("Long exposure set"));
			} else {
				gp_context_status (context, _("could not set Long exposure"));
			}
		}
	}

	gp_widget_get_child_by_label (window, _("Transfert Mode"), &w);
	if (gp_widget_changed (w)) {
		gp_widget_get_value (w, &ivalue);
		if (!check_readiness (camera, context)) {
			gp_context_status (context, _("Camera unavailable"));
...
camera_init (Camera *camera, GPContext *context)
{
	...
	fsfuncs.file_list_func = file_list_func;
	fsfuncs.folder_list_func = folder_list_func;
	fsfuncs.get_info_func = get_info_func;
	fsfuncs.get_file_func = get_file_func;
	fsfuncs.del_file_func = delete_file_func;
	fsfuncs.put_file_func = put_file_func;
	fsfuncs.make_dir_func = make_dir_func;
	fsfuncs.remove_dir_func = remove_dir_func;

	/* First, set up all the function pointers */
	camera->functions->exit = camera_exit;
	camera->functions->capture = camera_capture;
	camera->functions->capture_without_cf = camera_capture_without_cf;
...
//modif michel
   ...
   //camera->pl->transfert_mode =  REMOTE_CAPTURE_FULL_TO_DRIVE ;  // | REMOTE_CAPTURE_FULL_TO_PC REMOTE_CAPTURE_FULL_TO_DRIVE
   camera->pl->transfert_mode =  REMOTE_CAPTURE_FULL_TO_PC;
   camera->pl->timeValue = 1.0;
   camera->pl->driveMode = 0;
   strcpy(camera->pl->quality,"Small:Normal");
