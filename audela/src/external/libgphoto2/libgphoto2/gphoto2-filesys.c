			   ...
			   const char *filename, GPContext *context)
{
	...
	/* Delete the file */
   if (strcmp(folder,VIRTUAL_FOLDER)!=0) {
 ...
 **/
int
gp_filesystem_put_virtual_file (CameraFilesystem *fs, 
     CameraFile *file, GPContext *context)
{
	int x;
	...
	/* Search the folder */
	CR (x = gp_filesystem_folder_number (fs, VIRTUAL_FOLDER, context));
...
// modif michel
		fs->lru_first->lru_prev = fs->lru_first->lru_prev;
