...
#--- Begin change AudeLA : Delete "%1" for open file pdf
    if { [regexp {http:/localhost/cgi-bin/man/man2html\?(\w+)\+(\w+)} $new {} sec word] } {
        ...
        return
    } elseif { $new != "" && ( [regexp {[a-zA-Z]+[a-zA-Z]:.*} $new] || [ file extension $new ] == ".pdf" ) } {
        set url $new
        regexp {[^/\\]*:.*} $new url
        if { [regexp {:/[^/]} $url] } {
            regsub {:/} $url {://} url
        ...
        if { $tcl_platform(platform) != "windows"} {
            ...
            if { [info command freewrap::shell_getCmd_imp] ne "" } {
                set comm [freewrap::shell_getCmd_imp .html open]
            } else {
                set extension [ file extension $url ]
                set comm ""
                catch {
...
proc HelpViewer::ReadFile {name} {
    ...
    if { [package vcompare [package present Tkhtml] 3.0] < 0 } {
        ...
        regsub -all {\u00E9} $r {\&eacute;} r
            ...
            #--- Begin change AudeLA : Caracteres accentues
            set map {
                à &agrave;
                á &aacute;
                â &acirc;
...
#  user geom parameter
#  add accentuated characters

###package provide helpviewer 1.4

package provide helpviewer 1.4.1
