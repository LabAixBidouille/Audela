...
#
#      RCS:  $Id: bltHiertable.tcl,v 1.18 2000/06/23 00:40:05 gah Exp $
#
# ----------------------------------------------------------------------
...
proc blt::Hiertable::Init { widget } {
    ...
    $widget bin Entry <Shift-Double-ButtonPress-1> {
    ...
    $widget bin Entry <Control-Double-ButtonPress-1> {
    ...
    $widget bin Entry <Control-Shift-Double-ButtonPress-1> {
...
if { $tcl_platform(platform) == "windows" } {
    if { $tk_version >= 8.3 } {
	set cursor "@[file join $blt_library htresize.cur]"
    } else {
	set cursor "size_we"
    }
    option add *Hiertable.ResizeCursor $cursor
} else {
    option add *Hiertable.ResizeCursor \
	"@$blt_library/htresize.xbm $blt_library/htresize_m.xbm black white"
}
...
bind HiertableEditor <BackSpace> {
    blt::Hiertable::EditorBackspace [winfo parent %W]
}
...
bind HiertableEditor <KeyPress> {
    blt::Hiertable::Insert [winfo parent %W] %A
}
...
bind HiertableEditor <Escape> { 
    [winfo parent %W] text cancel 
}
bind HiertableEditor <Return> { 
    [winfo parent %W] text apply 
}
bind HiertableEditor <Shift-Return> {
    blt::Hiertable::Insert [winfo parent %W] "\n"
}
...
proc blt::Hiertable::EditorBackspace w {
