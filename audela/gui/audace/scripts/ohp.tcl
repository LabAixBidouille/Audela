		...
		foreach methode $methodes {
			...
			if {$methode=="bifsconv"} {
				...
				if {$n==0} {
					...
					foreach fic $fics {
						...
						set err [catch {buf1 load $fic2} msg]
			...
			if {$methode=="convert"} {
				...
				for {set k $kdeb} {$k<$kfin} {incr k} {
					...
					if {$telescope=="t80"} {
						...
						buf1 setkwd [list PIXSIZE1 $pixsize double "Pixel dimension" "m"]
						buf1 setkwd [list PIXSIZE2 $pixsize double "Pixel dimension" "m"]
						buf1 setkwd [list FOCLEN  $foclen    double "Focal length" "m"]
						buf1 setkwd [list RA $ra double "RA J2000.0" "deg"]
						buf1 setkwd [list DEC $dec double "DEC J2000.0" "deg"]
						buf1 setkwd [list CRVAL1 $ra double "" "deg"]
						buf1 setkwd [list CRVAL2 $dec double "" "deg"]
						buf1 setkwd [list CDELT1 [expr -$cdelt] double "X scale" "deg/pix"]
						buf1 setkwd [list CDELT2 $cdelt double "Y scale" "deg/pix"]
						buf1 setkwd [list CROTA2 $crota double "" "deg"]
						buf1 save $fullname2
						buf1 setkwd [list CRPIX1 [expr $naxis1/2] double "" "pix"]
						buf1 setkwd [list CRPIX2 [expr $naxis2/2] double "" "pix"]
					}
					if {$telescope=="t120"} {
						...
						buf1 setkwd [list CRPIX1 [expr $naxis1/2] double "" "pix"]
						buf1 setkwd [list CRPIX2 [expr $naxis2/2] double "" "pix"]
						set date_obs [lindex [buf1 getkwd DATE-OBS] 1]
						...
						if {$kk<0} {
							...
							set instant [lindex [buf1 getkwd TM-START] 1]
							...
							buf1 setkwd [list DATE-OBS $date_obs string "debut de pose" "iso8601"]
							set exposure [lindex [buf1 getkwd TM-EXPOS] 1]
							buf1 setkwd [list EXPOSURE $exposure float "duree de pose" "s"]
							set ra [lindex [buf1 getkwd POSTN-RA] 1]
							set dec [lindex [buf1 getkwd POSTN-DE] 1]
						} else {
							set exposure [lindex [buf1 getkwd EXPTIME] 1]
							buf1 setkwd [list EXPOSURE $exposure float "duree de pose" "s"]
						}
						buf1 setkwd [list RA $ra double "RA J2000.0" "deg"]
						buf1 setkwd [list DEC $dec double "DEC J2000.0" "deg"]
			...
			if {$methode=="astrometry"} {
				...
				for {set k $kdeb} {$k<=$kfin} {incr k} {
					...
					if {$erreur==1} { ::console::affiche_resultat " ERREUR : $msg\n" }
					buf1 load ${path}${shortname}${extout}
					set catastar [lindex [buf1 getkwd CATASTAR] 1]
			...
			if {$methode=="extraction_serie_spectro"} {
				...
				for {set k 1} {$k<=$nin} {incr k} {
					...
					buf1 load "${path}${namein}${k}.fit"
					set dateobs [lindex [buf1 getkwd DATE-OBS] 1]
					set exposure [expr [lindex [buf1 getkwd $expkwd] 1]/86400./2]
...
proc ohp_spectro_make_flat { namein nameout nin namedark telescope saturation } {
	...
	set sature yes
	if {$saturation=="no"} {
	   buf1 imaseries "BACK beck_kernel=15 back_threshold=0.4 div"
	} else {
	   set naxis1 [lindex [buf1 getkwd NAXIS1] 1]
	   set naxis2 [lindex [buf1 getkwd NAXIS2] 1]
	   if {$telescope=="t193"} {
	   ...
	   buf1 imaseries "BINX x1=$x1 x2=$x2 width=$naxis1"
	   if {$telescope=="t193"} {
		   ngain 15000
	   } else {
 			set box {11 365 136 574}
 			set res [buf1 stat $box]
...
proc ohp_spectro_pretraitement { namein nameout nin namedark nameflat telescope} {
	...
	if {$telescope=="t80"} {
		...
		for {set y $y1} {$y<=$y2} {incr y} {
			set p1 [buf1 getpix [list [expr $x-1] $y]]
			set p2 [buf1 getpix [list [expr $x+1] $y]]
			set p [expr ($p1+$p2)/2.]
			buf1 setpix [list $x $y] $p
		}
		uncosmic 0.9
		saveima $nameout
	}
	...
	for {set k 0} {$k<=$nin} {incr k} {
		...
		if {$telescope=="t80"} {
			...
			set flux [lindex [buf1 phot $box] 0]
			set boxcuts {437 457 862 532}
		} else {
			set flux ?
			set boxcuts {1 50 200 200}
		}
		set res [buf1 stat $boxcuts]
		visu visu1 [lrange $res 0 1]
...
proc ohp_spectro_profil { path name telescope } {
	...
	set naxis2 [lindex [buf1 getkwd NAXIS2] 1]
	if {$telescope=="t193"} {
		buf1 imaseries "BINX x1=1 x2=$naxis1 width=$naxis1"
		set box {21 307 104 600}
	} else {
		#buf1 imaseries "BINX x1=500 x2=600 width=$naxis1"
	...
	loadima $name
	buf1 imaseries "BINY y1=[expr int($yc-2*$yl)] y2=[expr int($yc+2*$yl)] heigth=10"
	buf1 bitpix -32
	saveima obj
	loadima $name
	buf1 imaseries "BINY y1=[expr int(4*$yl+$yc-2*$yl)] y2=[expr int(4*$yl+$yc+2*$yl)] heigth=10"
	buf1 bitpix -32
	saveima sky1
	loadima $name
	buf1 imaseries "BINY y1=[expr int(-4*$yl+$yc-2*$yl)] y2=[expr int(-4*$yl+$yc+2*$yl)] heigth=10"
	...
	for {set k 1} {$k<=$naxis1} {incr k} {
		lappend xs $k
		lappend valeurs [buf1 getpix [list $k 1]]
	}
...
proc ohp_spectro_extract { path namein nameout telescope} {
	...
	loadima $namein
	set texp [lindex [buf1 getkwd EXPOSURE] 1]
	if {$texp==""} {
		set texp [lindex [buf1 getkwd EXPTIME] 1]
	}
	set ra [lindex [buf1 getkwd RA] 1]
	set dec [lindex [buf1 getkwd DEC] 1]
	set jdbeg [mc_date2jd [lindex [buf1 getkwd DATE-OBS] 1]]
