...
proc setup { } {
   ...
   set err [catch {
      ...
      set telscript($telname,combit0) [open COM1 "RDWR"]
      fconfigure $telscript($telname,combit0) -mode "9600,n,8,1" -buffering none -blocking 0
      set telscript($telname,combit1) [open COM2 "RDWR"]
   ...
   if {$telscript($telname,mount_type)=="azelevrot"} {
      ...
      set err [catch {etel_open -driver DSTEB3 -axis 0 -axis 2} msg]
   } elseif {$telscript($telname,mount_type)=="hadec"} {
      set err [catch {etel_open -driver DSTEB3 -axis 0 -axis 1} msg]
   } else {
      set err 1
   }
   if {$err==0} {
   ...
   if {$telscript($telname,simulation)==0} {
      ...
      if {($telscript($telname,mount_type)=="azelevrot")||($telscript($telname,mount_type)=="azelev")} {
         ...
         #set telscript($telname,coord_app_adu_az0) 672077188 ; # position init (adu)
         set telscript($telname,coord_app_adu_az0) 678849285 ; # position init (adu)
         set telscript($telname,coord_app_deg_az0) 0 ; # position init (deg)
         ...
         set telscript($telname,adu4deg_az)        [expr -1*$telscript($telname,adu4deg_az)]
         # --- elev
         set telscript($telname,adu4deg4sec_elev)    7817302 ; # vitesse coef (adu/(deg/s))
         set telscript($telname,adu4deg_elev)         14540000 ; # position coef (adu/deg)
         #set telscript($telname,coord_app_adu_elev0)180294334 ; # position init (adu)
         set telscript($telname,coord_app_adu_elev0) [expr 132000000-1*100000] ; # position init (adu)
         set telscript($telname,coord_app_deg_elev0) 0 ; # position init (deg)
         set telscript($telname,lim_min_elev)        [expr 305544000+180294334-246229900 ] ; # limite inf adu
