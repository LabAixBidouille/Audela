...
namespace eval OuranosCom {
   ...
   proc go_ouranos { } {
      ...
      global frmm
         ...
         if { $confTel(conf_ouranos,find) == "1" } {
            set confTel(conf_ouranos,find) "0"
   ...
   proc match_ouranos { } {
      ...
      #--- Envoi le Match
      tel$audace(telNo) radec init { $confTel(conf_ouranos,match_ra) $confTel(conf_ouranos,match_dec) }
      return
   }
   ...
   proc match_transfert_ouranos { } {
      ...
      set frm $frmm(Telscp2)
      if { $confTel(conf_ouranos,obj_choisi) == "$caption(ouranoscom,messier)" } {
         set confTel(conf_ouranos,match_ra) $catalogue(objet_ad)
         $frm.match_ra_entry configure -textvariable confTel(conf_ouranos,match_ra)
         set confTel(conf_ouranos,match_dec) $catalogue(objet_dec)
         $frm.match_dec_entry configure -textvariable confTel(conf_ouranos,match_dec)
      } elseif { $confTel(conf_ouranos,obj_choisi) == "$caption(ouranoscom,ngc)" } {
         set confTel(conf_ouranos,match_ra) $catalogue(objet_ad)
         $frm.match_ra_entry configure -textvariable confTel(conf_ouranos,match_ra)
         set confTel(conf_ouranos,match_dec) $catalogue(objet_dec)
         $frm.match_dec_entry configure -textvariable confTel(conf_ouranos,match_dec)
      } elseif { $confTel(conf_ouranos,obj_choisi) == "$caption(ouranoscom,ic)" } {
         set confTel(conf_ouranos,match_ra) $catalogue(objet_ad)
         $frm.match_ra_entry configure -textvariable confTel(conf_ouranos,match_ra)
         set confTel(conf_ouranos,match_dec) $catalogue(objet_dec)
         $frm.match_dec_entry configure -textvariable confTel(conf_ouranos,match_dec)
      } elseif { $confTel(conf_ouranos,obj_choisi) == "$caption(ouranoscom,etoile)" } {
         set confTel(conf_ouranos,match_ra) $catalogue(etoile_ad)
         $frm.match_ra_entry configure -textvariable confTel(conf_ouranos,match_ra)
         set confTel(conf_ouranos,match_dec) $catalogue(etoile_dec)
         $frm.match_dec_entry configure -textvariable confTel(conf_ouranos,match_dec)
      }
   ...
   proc init_ouranos { } {
      ...
      #---
      set confTel(conf_ouranos,status)      $caption(ouranoscom,off)
      #---
      set confTel(conf_ouranos,cod_dec)     $conf(ouranos,cod_dec)	  
      set confTel(conf_ouranos,cod_ra)      $conf(ouranos,cod_ra)
      set confTel(conf_ouranos,freq)        $conf(ouranos,freq)
      set confTel(conf_ouranos,init)        $conf(ouranos,init)
      set confTel(conf_ouranos,inv_dec)     $conf(ouranos,inv_dec)
	set confTel(conf_ouranos,inv_ra)      $conf(ouranos,inv_ra)
      set confTel(conf_ouranos,show_coord)  $conf(ouranos,show_coord)
      set confTel(conf_ouranos,tjrsvisible) $conf(ouranos,tjrsvisible)
      #---
      set confTel(conf_ouranos,find)        "0"
   ...
   proc set_dec_ra { } {
	...
	if { [ string compare $confTel(conf_ouranos,init) "1" ] == "0" } {
          set confTel(conf_ouranos,init_dec) [ expr $confTel(conf_ouranos,cod_dec)/4 ]
      } else {
          set confTel(conf_ouranos,init_dec) [ expr $confTel(conf_ouranos,cod_dec)/2 ]
      }  
      #--- Preposition de RA
      set confTel(conf_ouranos,init_ra) [ expr $confTel(conf_ouranos,cod_ra)/2 ]
      return
   }
      ...
      if { [ string compare $ouranoscom(tty) "" ] != "0" } {
	   ...
	   #--- Procede a une lecture
         ...
         if { [ string compare $answer "" ] != "0" } {
	      ...
	      if { [ string compare $ouranoscom(trace_in) "0"] != "0" } {
               console::affiche_erreur "$caption(ouranoscom,dec1) $caption(ouranoscom,2points)\
                  $confTel(conf_ouranos,dec_enc) $caption(ouranoscom,ad1) $caption(ouranoscom,2points)\
                  $confTel(conf_ouranos,ra_enc)\n"			
	      }
	   } else {
            ::OuranosCom::no_answer
         }
         if { [ string compare $confTel(conf_ouranos,find) "0" ] == "0" } {
            ::OuranosCom::show1
         } else {
            ::OuranosCom::show2
         }
         ...
         if { [ winfo exists $audace(base).tjrsvisible ] } {
            if { $confTel(conf_ouranos,tjrsvisible) == "1" } {
               set Fenetre(conf_ouranos,coord_ra) "$caption(ouranoscom,ad) $confTel(conf_ouranos,coord_ra)"
               $audace(base).tjrsvisible.lab1 configure -text "$Fenetre(conf_ouranos,coord_ra)"
               set Fenetre(conf_ouranos,coord_dec) "$caption(ouranoscom,dec) $confTel(conf_ouranos,coord_dec)"
               $audace(base).tjrsvisible.lab2 configure -text "$Fenetre(conf_ouranos,coord_dec)"
            }
         } elseif { [ winfo exists $audace(base).tjrsvisible_x10 ] } {
            if { $confTel(conf_ouranos,tjrsvisible) == "1" } {
               set Fenetre(conf_ouranos,coord_ra) "$caption(ouranoscom,ad)\n$confTel(conf_ouranos,coord_ra)"
               $audace(base).tjrsvisible_x10.lab1 configure -text "$Fenetre(conf_ouranos,coord_ra)"
               set Fenetre(conf_ouranos,coord_dec) "$caption(ouranoscom,dec)\n$confTel(conf_ouranos,coord_dec)"
               $audace(base).tjrsvisible_x10.lab2 configure -text "$Fenetre(conf_ouranos,coord_dec)"
            }
         } else {
            set confTel(conf_ouranos,tjrsvisible) "0"
         }
         if { $ouranoscom(lecture) == "1" } {
	      after [ expr $confTel(conf_ouranos,freq)*1000 ] ::OuranosCom::read_coord
         } else {
   ...
   proc show1 { } {
	...
	global confTel
     	   ...
     	   if { [ string compare $confTel(conf_ouranos,show_coord) "0" ] == "0" } {
            set pas_encod [ tel$audace(telNo) nbticks ]
            set confTel(conf_ouranos,coord_ra) "[ lindex $pas_encod 0 ] $caption(ouranoscom,pas)"
            set confTel(conf_ouranos,coord_dec) "[ lindex $pas_encod 1 ] $caption(ouranoscom,pas)"
         } else {
            set radec [ tel$audace(telNo) radec coord ]
            set confTel(conf_ouranos,coord_ra) [ lindex $radec 0 ]
	      set confTel(conf_ouranos,coord_dec) [ lindex $radec 1 ]
   ...
   proc show2 { } {
	global confTel
      ...
      set dec_enc [ expr $confTel(conf_ouranos,dec_enc) ]
      set ra_enc [ expr $confTel(conf_ouranos,ra_enc) ]
      if { [ expr abs($dec_enc)-32768 ] > "0" } {
         set dec_enc [ expr 65536-[ expr abs($dec_enc) ] ]
         set confTel(conf_ouranos,inv_dec) "-1"
      }
      if { [ expr abs($ra_enc)-32768 ] > "0" } {
         set ra_enc [ expr 65536-[ expr abs($ra_enc) ] ]
         set confTel(conf_ouranos,inv_ra) "-1"
      }
      set confTel(conf_ouranos,cod_dec) $dec_enc
      set confTel(conf_ouranos,cod_ra) $ra_enc
      return
   }
   ...
   proc analyze_answer { answer } {
	...
	set confTel(conf_ouranos,dec_enc) [ format "%g" [ lindex $coords 1 ] ]
     	set confTel(conf_ouranos,ra_enc) [ format "%g" [ lindex $coords 0 ] ]
     	set confTel(conf_ouranos,dec_enc) [ expr $confTel(conf_ouranos,inv_dec)*$confTel(conf_ouranos,dec_enc) ]
      set confTel(conf_ouranos,ra_enc) [ expr $confTel(conf_ouranos,inv_ra)*$confTel(conf_ouranos,ra_enc) ]
      return
   }
   ...
   proc close_com { } {
	global caption
      ...
      set frm $frmm(Telscp2)
         ...
         set confTel(conf_ouranos,match_ra) ""
         set confTel(conf_ouranos,match_dec) ""
         destroy $frm.match_ra
         destroy $frm.match_dec
         destroy $frm.match_ra_entry
         destroy $frm.match_dec_entry
         if { ( $confTel(conf_ouranos,show_coord) == "1" ) && ( $conf(ouranos,show_coord) == "1" ) } {
	      #--- Valeur Dec. en ° ' "
   ...
   proc find_res { } {
      ...
      global frmm
	   ...
	   $frm.but_read configure -text "$caption(ouranoscom,lire)" -state disabled
         ...
         update
         set confTel(conf_ouranos,cod_dec)    "65536"	  
         set confTel(conf_ouranos,cod_ra)     "65536"
         set confTel(conf_ouranos,init_dec)   "0"	  
         set confTel(conf_ouranos,init_ra)    "0"	  
	   set confTel(conf_ouranos,init)       "0"
         set confTel(conf_ouranos,inv_dec)    "1"
         set confTel(conf_ouranos,inv_ra)     "1"
         set confTel(conf_ouranos,show_coord) "0"
         ::confTel::MatchOuranos
         set confTel(conf_ouranos,find)       "1"
	   ::OuranosCom::send_encod [ format "R%d\t%d\r" $confTel(conf_ouranos,cod_ra) $confTel(conf_ouranos,cod_dec) ]
	   ::OuranosCom::send_encod [ format "I%d\t%d\r" $confTel(conf_ouranos,init_ra) $confTel(conf_ouranos,init_dec) ]
         ::OuranosCom::read_coord
      }
      return
   }
   ...
   proc TjrsVisible { } {
      ...
      global caption
         ...
         if { $confTel(conf_ouranos,tjrsvisible) == "0" } {
            destroy $audace(base).tjrsvisible
         } else {
            if { [ winfo exists $audace(base).tjrsvisible ] } {
               destroy $audace(base).tjrsvisible
            ...
            radiobutton $audace(base).tjrsvisible.rad0 -anchor nw -highlightthickness 0 -padx 0 -pady 0 \
               -text "$caption(ouranoscom,x1)" -value 0 -variable confTel(conf_ouranos,dim) -command {
            ...
            radiobutton $audace(base).tjrsvisible.rad1 -anchor nw -highlightthickness 0 -padx 0 -pady 0 \
               -text "$caption(ouranoscom,x5)" -value 1 -variable confTel(conf_ouranos,dim) -command {
      ...
      global caption
         ...
         if { $confTel(conf_ouranos,tjrsvisible) == "0" } {
            ...
            radiobutton $audace(base).tjrsvisible_x10.rad0 -anchor nw -highlightthickness 0 -padx 0 -pady 0 \
               -font {verdana 20 bold} -text "$caption(ouranoscom,:5)" -value 0 -variable confTel(conf_ouranos,dim) \
               -command {
                  destroy $audace(base).tjrsvisible_x10 ; ::OuranosCom::TjrsVisible
               }
            ...
            radiobutton $audace(base).tjrsvisible_x10.rad1 -anchor nw -highlightthickness 0 -padx 0 -pady 0 \
               -font {verdana 20 bold} -text "$caption(ouranoscom,x1)" -value 1 -variable confTel(conf_ouranos,dim) \
               -command {
                  destroy $audace(base).tjrsvisible ; ::OuranosCom::TjrsVisible_x10
               }
