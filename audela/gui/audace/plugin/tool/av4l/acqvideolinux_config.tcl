...
namespace eval acqvideolinux_config {
   ...
   #
   #--------------------------------------------------
   proc ::acqvideolinux_config::install { } {

      global acqvideolinux

      set acqvideolinux(withtk) 1
      source [file join $::audela_start_dir acqvideolinux_install.tcl]
      ::acqvideolinux_install::run
   }
   ...
   proc createDialog { } {
      ...
      # Charge les config acqvideolinux depuis le fichier XML
      set err [::acqvideolinuxXML::load_xml_config]
      # Recupere la liste des acqvideolinux disponibles
      set acqvideolinuxconf(list_config) $::acqvideolinuxXML::list_acqvideolinux
      # Recupere la config par defaut [liste id name]
      set acqvideolinuxconf(default_config) $::acqvideolinuxXML::default_config
      # Recupere la config par courante [liste id name]
      set acqvideolinuxconf(current_config) $::acqvideolinuxXML::current_config
