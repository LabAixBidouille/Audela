#
# Mise à jour $Id: tuto.plug3.tcl,v 1.9 2010-05-23 08:28:58 robertdelmas Exp $
#
...
"
   ...
   set texte(firstdark_3) "\
On the photo, the computer is linked to a hub by a non-crossover cable.\
The EthernAude device is linked to another socket of the hub by\
another non-crossover cable.\
The EthernAude device is power supplied by an external stabilized supply box\
(9 or 12V for 1 A max).\
The multicolored flat cable is plugged in the EthernAude device and\
in the Genesis/Audine camera. The camera is connected\
to its supply box (right on the photo).\n\
\n\
It is strongly recommended to choose a switch rather than a hub.\
This is to limit network collisions that can generate\
...
wm protocol .second WM_DELETE_WINDOW tuto_plug3_exit
set widgetDemo 1
...
.second.t insert end " \n " {demospace}
.second.t insert end "$texte(next_topic) $texte(tuto_8)" {demo demo-on}
.second.t insert end " \n " {demospace}
.second.t insert end " \n " {demospace}
