...
proc caption_def { langage } {
   ...
   if {[string compare $langage french] ==0 } {
     set caption(main_title) "Tuto : Un tutorial pour les debutants en CCD"   
     set caption(tuto_about) "A propos..."
     set caption(tuto_quit)  "Quitter"
     set caption(tuto_tools) "Outils"
     set texte(tuto_1)    "Initiation a l'imagerie CCD"
     set texte(tuto_2)    "Ce tutorial vous permettra de realiser vos premieres images avec une camera CCD. Il a ete realise avec le logiciel AudeLA."
     set texte(tuto_3)    "Brancher et tester la camera Audine"
     set texte(tuto_4)    "Connexion des cables de la camera."
     set texte(tuto_5)    "Tests electroniques pour le kit Audine."
     set texte(tuto_6)    "Premiere image dans le noir, camera non-refroidie."
     set texte(tuto_7)    "Premiere lumiere, camera non-refroidie."
     set texte(tuto_8)    "Refroidissement de la camera."
     set texte(tuto_9)    "Realiser des images dark et bias de reference."
     set texte(tuto_10_0) "Installer la camera CCD sur un telescope"
     set texte(tuto_10)   "Bague pour adapter Audine sur un porte-oculaire."
     set texte(tuto_11)   "Adaptation mecanique sur le porte-oculaire."
     set texte(tuto_12)   "Mise en station du telescope."
     set texte(tuto_13)   "Pointage des objets celestes."
     set texte(tuto_14_0) "Premieres images sur le ciel"
     set texte(tuto_14)   "Focalisation sur une etoile brillante."
